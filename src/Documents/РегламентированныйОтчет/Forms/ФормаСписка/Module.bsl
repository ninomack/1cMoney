&НаКлиенте
Перем ВидОтчетаДляАктивизацииВIE;
...
Процедура УстановитьОтборПоОрганизации()
	...
	ОтборОрганизация.Использование = Ложь; // ДЕНЬГИ Истина;
...
Процедура ПриСозданииНаСервере()
	...
	// ДЕНЬГИ
	// удалено определение учета по организациям
...
Процедура ДобавлениеОтчетаВЖурналеОтчетов()
	...
	СписокОтчетов.Добавить("3-НДФЛ", "Декларация 3-НДФЛ");
	СписокОтчетов.Добавить("4-НДФЛ", "Декларация 4-НДФЛ");
	Оповещение = Новый ОписаниеОповещения("ВыборВидаОтчетаЗавершение", ЭтотОбъект);
	СписокОтчетов.ПоказатьВыборЭлемента(Оповещение, НСтр("ru = 'Укажите вид декларации'"));
	Возврат;
	// Конец ДЕНЬГИ
...
&НаКлиенте
Процедура ПодождатьДляIE()
	
	ОтключитьОбработчикОжидания("ПодождатьДляIE");
	Оповестить(ВидОтчетаДляАктивизацииВIE, "Активизировать");
...
&НаКлиенте
Процедура ВыборВидаОтчетаЗавершение(Выбор, ДополнительныеПараметры) Экспорт

	Если Выбор = Неопределено Тогда
		Возврат;
	ИначеЕсли Выбор.Значение = "3-НДФЛ" Тогда
		ОткрытьФорму("Отчет.РегламентированныйОтчет3НДФЛ.Форма.ОсновнаяФорма", Новый Структура("Организация"));
	ИначеЕсли Выбор.Значение = "4-НДФЛ" Тогда
		ОткрытьФорму("Отчет.РегламентированныйОтчет4НДФЛ.Форма.ОсновнаяФорма", Новый Структура("Организация"));
	КонецЕсли; 
	Возврат;
...
&НаСервереБезКонтекста
Функция СсылкаСправочникаРегОтчетов(НаименованиеОтчета)
	
	Возврат Справочники.РегламентированныеОтчеты.НайтиПоНаименованию(НаименованиеОтчета);
	
КонецФункции 

&НаКлиенте
Процедура Создать3НДФЛ(Команда)
	
	НаименованиеОтчета = "Декларация 3-НДФЛ";
	СсылкаВидОтчета = СсылкаСправочникаРегОтчетов(НаименованиеОтчета);
	СтруктураОтчета = Новый Структура("ИсточникОтчета,Наименование,Ссылка", 
			"РегламентированныйОтчет3НДФЛ", НаименованиеОтчета, СсылкаВидОтчета);
	ОткрытьФормуОтчетаНаКлиенте(СтруктураОтчета);
	//ОткрытьФорму("Отчет.РегламентированныйОтчет3НДФЛ.Форма.ОсновнаяФорма", Новый Структура("Организация"));
	
КонецПроцедуры

&НаКлиенте
Процедура Создать4НДФЛ(Команда)
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Записан 3-НДФЛ" Тогда
