...
Процедура УстановитьОтборПоВидуОтчета(ВидОтчета)
	...
	// ДЕНЬГИ
	// Уаделение устаревшей функции БСП:
...
Процедура ДобавлениеОтчетаВЖурналеОтчетов()
	...
	Оповещение = Новый ОписаниеОповещения("ДобавлениеОтчетаВЖурналеОтчетовЗавершение", ЭтотОбъект, ДополнительныеПараметры); 
	СписокОтчетов.ПоказатьВыборЭлемента(НСтр("ru = 'Укажите вид декларации'"));
	// Конец ДЕНЬГИ
...
Процедура ОткрытьФормуОтчетаНаКлиенте(Отчет, ОрганизацияОтчета = Неопределено, ДатаНачалаПериодаОтчета = Неопределено, ДатаОкончанияПериодаОтчета = Неопределено, ПериодичностьОтчета = Неопределено)
	...
	ИначеЕсли РезультатОткрытияФормыНаСервере = "Открыть внутренний отчет-документ" Тогда
		...
		Если Отчет.ИсточникОтчета = "РегламентированныйОтчетРСВ1" Тогда
			ОткрытьФорму("Документ." + Отчет.ИсточникОтчета + ".Форма." + РегламентированнаяОтчетностьКлиентСерверПереопределяемый.ИмяОсновнойФормыРСВ1(), , , Отчет.ИсточникОтчета);		
		Иначе
	...
	ИначеЕсли ТипЗнч(РезультатОткрытияФормыНаСервере) = Тип("Структура") Тогда
		...
		Если Отчет.ИсточникОтчета = "РегламентированныйОтчетРСВ1" Тогда
			ОткрытьФорму("Отчет." + Отчет.ИсточникОтчета + ".Форма." + РегламентированнаяОтчетностьКлиентСерверПереопределяемый.ИмяОсновнойФормыРСВ1(), РезультатОткрытияФормыНаСервере);
		Иначе
...
&НаКлиенте
Процедура НастроитьПериодЗавершение(Ответ, ДополнительныеПараметры) Экспорт

	Если Ответ <> Неопределено Тогда
		ДатаНачалаПериодаОтчета = Ответ.Период.ДатаНачала;
		ДатаКонцаПериодаОтчета  = Ответ.Период.ДатаОкончания;
...
&НаКлиенте
Процедура ДобавлениеОтчетаВЖурналеОтчетовЗавершение(Выбор, ДополнительныеПараметры) Экспорт
