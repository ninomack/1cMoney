...
Процедура ЗагрузитьАктуальныйКурс(ПараметрыЗагрузки = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	// ДЕНЬГИ
	// Для более дружественного информирования пользователя запомним валюты с ошибками
	Результат = Новый Массив;
	// Конец ДЕНЬГИ
	...
	Если ПараметрыЗагрузки = Неопределено Тогда
		...
		// ДЕНЬГИ
		|	И КурсыВалют.Валюта.Активность
		// Конец ДЕНЬГИ
		|
		|СГРУППИРОВАТЬ ПО
		|	КурсыВалют.Валюта,
		...
		Пока Выборка.Следующий() Цикл
			...
			КонецПериодаПоВалюте = Макс(КонецДня(ТекущаяДата) + 1, НачалоПериода);
			МассивСостояния = ЗагрузитьКурсыВалютПоПараметрам(СписокВалют, НачалоПериода, КонецПериодаПоВалюте, ПриЗагрузкеВозниклиОшибки);
			Для каждого ЭлементРезультат Из МассивСостояния Цикл
				Результат.Добавить(ЭлементРезультат);
			КонецЦикла;
			// Конец ДЕНЬГИ
	...
	Если ПриЗагрузкеВозниклиОшибки Тогда
		...
		Для каждого ЭлементРезультат Из Результат Цикл
			Если Не ЭлементРезультат.СтатусОперации Тогда
				МассивНезагруженных.Добавить(ЭлементРезультат);
			КонецЕсли;
		КонецЦикла;
		
		Если МассивНезагруженных.Количество() = Результат.Количество() Тогда
			ТекстОшибки = НСтр("ru='Загрузка курсов не выполнена.'"); 
		Иначе
			
			ТекстОшибки = НСтр("ru='Курсы загружены не полностью. 
				|Не загружены курсы валют:'"); 
			Для каждого ЭлементРезультат Из МассивНезагруженных Цикл
				ТекстОшибки = ТекстОшибки + " " + ЭлементРезультат.Валюта;
			КонецЦикла;
		...
		ЗаписьЖурналаРегистрации(
			...
			НСтр("ru = 'Во время регламентного задания загрузки курсов валют возникли ошибки'") + Символы.ПС + ТекстОшибки);
			// Конец ДЕНЬГИ
...
Функция ЗагрузитьКурсыВалютПоПараметрам(Знач Валюты, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки, ПриЗагрузкеВозниклиОшибки = Ложь)
	...
	Для Каждого Валюта Из Валюты Цикл
		...
		Если Валюта.КодВалюты = "643" Тогда
			// если выбрана иная базовая валюта, должна быть визуальная возможность "загрузить курс рубля"
			Продолжить; 
		КонецЕсли; 
		// Конец ДЕНЬГИ
		...
		// ДЕНЬГИ
		// Если хотя бы одна валюта не загрузится, будет выдано сообщение об ошибке но без информации о причинах
		Если ЗначениеЗаполнено(ПоясняющееСообщение) Тогда
			ЗаписьЖурналаРегистрации(
				...
				, 
				,
				СтрШаблон(НСтр("ru = 'Ошибка загрузки курса для валюты %1: %2'"), Строка(Валюта.Валюта), ПоясняющееСообщение));
		КонецЕсли;
		// Конец ДЕНЬГИ
	...
	ОбщегоНазначенияДеньги.ПроверитьРазрывыКурсовВалют(?(ЗначениеЗаполнено(НачалоПериодаЗагрузки), НачалоМесяца(НачалоПериодаЗагрузки), НачалоПериодаЗагрузки), ОкончаниеПериодаЗагрузки);
	// Конец ДЕНЬГИ
...
Функция ЗагрузитьКурсВалютыИзФайла(Знач Валюта, Знач ПутьКФайлу, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки)
	...
	ЧислоЗагружаемыхДнейВсего = Цел(ЧислоЗагружаемыхДнейВсего);
	...
	Для НомерСтроки = 1 По Текст.КоличествоСтрок() Цикл
		...
		// ДЕНЬГИ
		Запись.БазоваяВалюта = БазоваяВалютаФайла;
		// Конец ДЕНЬГИ
	...
	Если ЧислоЗагружаемыхДнейВсего > ЧислоЗагруженныхДней И ЧислоЗагружаемыхДнейВсего > ЧислоЗагруженныхДней + 1 Тогда
	// Конец ДЕНЬГИ
