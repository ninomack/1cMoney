...
Функция НоваяСтруктураПериода(ВидПериода, НачалоПериода = Неопределено, КонецПериода = Неопределено)
	...
	ИначеЕсли ВидПериода = "НачалоПроизвольнойДаты" Или ВидПериода = "КонецПроизвольнойДаты" Тогда
		...
		Результат.ДатаНачала = ?(ЗначениеЗаполнено(Результат.ДатаНачала), Результат.ДатаНачала, НачалоМесяца(ТекущаяДатаСеанса()));
	...
	ИначеЕсли ВидПериода = "ПоследняяНеделя" Тогда
		...
		Результат.ДатаОкончания = КонецДня(ТекущаяДатаСеанса());
...
Функция НовыеНастройкиИндикаторов() Экспорт
	...
	Настройки.Вставить("НачалоПериодаБюджета", БюджетированиеКлиентСервер.КалендарноеНачалоПериода(ТекущаяДатаСеанса(), Настройки.ВидПериодаБюджета));
	Настройки.Вставить("КонецПериодаБюджета", БюджетированиеКлиентСервер.КалендарноеОкончаниеПериода(ТекущаяДатаСеанса(), Настройки.ВидПериодаБюджета));
...
Функция ПредставлениеИндикатораДинамики(ТипИндикатора, ВидОбъектовУчета) Экспорт
	...
	ИначеЕсли ТипИндикатора = "РежимРаботы" Тогда
		Результат = НСтр("ru='Режим синхронизации'");
	ИначеЕсли ТипИндикатора = "Планы" Тогда
...
// Возвращает текущую версию обработки (число) для автоматического обновления настроек
//
//Параметры:
//	Нет
//
...
Функция ВыборкиЗапросаПоказателей(НастройкиИндикаторов)
	...
	ДополнитьРезультатыРежимомРаботы(Результат, НастройкиИндикаторов);
...
Процедура ДополнитьТекстЗапросаКурсамиВалют(Запрос, СтруктураЗапроса, ОписаниеТипаЗапроса, НастройкиИндикаторов)
	...
	Запрос.УстановитьПараметр("ВалютаОкончаниеПериода", КонецДня(ТекущаяДатаСеанса()) + 1); // Всегда на 1 день вперед, чтобы иметь свежий курс на графике
...
Процедура ДополнитьТекстЗапросаБюджетом(Запрос, СтруктураЗапроса, ОписаниеТипаЗапроса, НастройкиИндикаторов)
	...
	Запрос.УстановитьПараметр("ИндикаторБюджета_ДатаПоследнихКурсов",    НачалоДня(ТекущаяДатаСеанса()));
...
Процедура ДополнитьТекстЗапросаИсторией(Запрос, СтруктураЗапроса, ОписаниеТипаЗапроса, НастройкиИндикаторов)
	...
	Запрос.УстановитьПараметр("КонечнаяДатаИстории",   КонецДня(ТекущаяДатаСеанса()));
...
Процедура ПроверитьПараметрыТипаЗапроса(ОписаниеТипаЗапроса, СтрокаНастройки, НастройкиИндикаторов)
	...
	ИначеЕсли СтрокаНастройки.ТипИндикатора = "ВводОпераций" Тогда
		...
		ПроверитьЗначениеПараметраЗапроса(ОписаниеТипаЗапроса.Параметры, "ПланыМаксимальнаяДата", КонецДня(ТекущаяДатаСеанса()), Ложь);
...
Процедура ЗаполнитьСписокИндикаторовДинамикиПоУмолчанию(СписокИндикаторов)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ФункционалКонфигурацииДеньги.ОбменСЦентральнойБазой") Тогда
		СтрокаНастройки = НоваяНастройкаИндикатораДинамики(СписокИндикаторов, "РежимРаботы", "Режим синхронизации");
	КонецЕсли;
...
Процедура ДополнитьРезультатыРежимомРаботы(Результат, НастройкиИндикаторов) 
	
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ФункционалКонфигурацииДеньги.ОбменСЦентральнойБазой") Тогда
		Возврат;
	КонецЕсли;
	
	Модуль = ОбщегоНазначения.ОбщийМодуль("ДеньгиОбменСЦентральнойБазой");
	Модуль.ДополнитьПоказателиНачальнойСтраницы(Результат, НастройкиИндикаторов);
