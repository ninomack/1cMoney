...
&НаКлиенте 
Перем ОткрытиеПриложенияОбработано;
...
Процедура ПриОткрытии(Отказ)
	...
	ОткрытиеПриложенияОбработано = Ложь;
	ПоказатьСостояниеОбновления("Старт");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ЗагрузкаКурсовВалют" Или ИмяСобытия = "Записана операция" Или ИмяСобытия = "Записан элемент справочника" 
		...
		Или ИмяСобытия = "ВыполненОбменДанными" Или ИмяСобытия = "Изменился список операций на период" 
		Или ИмяСобытия = "Изменилась дата плановой операции"
		 Тогда
	...
	ИначеЕсли ИмяСобытия = "ОформленаПодписка" Или ИмяСобытия = "ПодключениеКОблакуИзменено"
		Или ИмяСобытия = "ИзмененаСинхронизация" Тогда
...
Процедура ДекорацияФормыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли СтруктураЭлемента.Ссылка = "ВключитьСинхронизацию" Тогда
		ОткрытьФормуПодсистемыСинхронизацииСЦБ("ОбщаяФорма.ВыборСпособаСинхронизации");
	ИначеЕсли СтруктураЭлемента.Ссылка = "ОткрытьМониторСинхронизации" Тогда
		ОткрытьФорму("Обработка.МониторСинхронизацииСМобильнымиУстройствами.Форма.Монитор");
	ИначеЕсли СтруктураЭлемента.Ссылка = "ОткрытьСинхронизациюСОблаком" Тогда
		ОткрытьФормуПодсистемыСинхронизацииСЦБ("ОбщаяФорма.УправлениеСервисомАвтосинхронизации");
	ИначеЕсли СтруктураЭлемента.Ссылка = "ОткрытьПодключениеКОблаку" Тогда
		ОткрытьФормуПодсистемыСинхронизацииСЦБ("ОбщаяФорма.УправлениеСервисомАвтосинхронизации");
	ИначеЕсли СтруктураЭлемента.Ссылка = "ВыбратьСинхронизациюСОблаком" Тогда
		ВключитьОбменНаСервере(Истина);
		ОбновитьИнтерфейс();
		Оповестить("ИзмененаСинхронизация");
		ОткрытьФормуПодсистемыСинхронизацииСЦБ("ОбщаяФорма.УправлениеСервисомАвтосинхронизации");
	ИначеЕсли СтруктураЭлемента.Ссылка = "УправлениеПодписками" Тогда
		ОткрытьФормуПодсистемыСинхронизацииСЦБ("ОбщаяФорма.УправлениеСервисомАвтосинхронизации");
	Иначе
...
Процедура ОбновитьЗначенияВНастройках(НастройкиИндикаторов)
	...
	Если ЗначениеЗаполнено(НастройкиИндикаторов.ВариантБюджета) И ЗначениеЗаполнено(НастройкиИндикаторов.ВариантБюджета.ДетальностьПланирования) Тогда
		...
		ПериодПланирования = БюджетированиеСервер.НоваяСтрукрураБюджетногоПериода(НастройкиИндикаторов.ВариантБюджета, ТекущаяДатаКлиентСервер());
	...
	Иначе
		...
		НастройкиИндикаторов.Вставить("НачалоПериодаБюджета", НачалоМесяца(ТекущаяДатаКлиентСервер()));
	...
	Для каждого СтрокаНастроек Из НастройкиИндикаторов.ИндикаторыДинамики Цикл
		...
		Если СтрокаНастроек.ТипИндикатора = "РежимРаботы" Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПроверитьНастройкиИндикаторов(НастройкиИндикаторов)
	...
	ТекВерсия      = Обработки.НачальнаяСтраница.ВерсияОбработки();
	ВерсияНастроек = ?(НастройкиИндикаторов.Свойство("Версия"), НастройкиИндикаторов.Версия, 1);
	Пока ВерсияНастроек < ТекВерсия Цикл
		
		Если ВерсияНастроек = 1 Тогда
			ДобавитьИндикаторСостояния(НастройкиИндикаторов);
		КонецЕсли;
		
		ВерсияНастроек = ВерсияНастроек + 1;
		НастройкиИндикаторов.Вставить("Версия", ВерсияНастроек);
	КонецЦикла;
	...
	Для каждого НастройкаСобытия Из НастройкиИндикаторов.ИндикаторыДинамики Цикл
		...
		Если НастройкаСобытия.ТипИндикатора = "ВводОпераций" Тогда
			НастройкаСобытия.Период = НоваяСтруктураПериода("НачалоПроизвольнойДаты", НачалоДня(ТекущаяДатаКлиентСервер()), КонецДня(ТекущаяДатаКлиентСервер()));
		КонецЕсли;
		
		Если НастройкаСобытия.ТипИндикатора = "РежимРаботы" И Не ИспользуетсяОбменСЦБ Тогда
			УдаляемыеНастройки.Добавить(НастройкаСобытия);
		КонецЕсли;
	...
	Для каждого УдаляемаяСтрока Из УдаляемыеНастройки Цикл
		НастройкиИндикаторов.ИндикаторыДинамики.Удалить(УдаляемаяСтрока);
	КонецЦикла;
...
&НаСервере
Процедура ДобавитьИндикаторСостояния(НастройкиИндикаторов)
	
	Если Не ИспользуетсяОбменСЦБ Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаНастройки = НастройкиИндикаторов.ИндикаторыДинамики.Вставить(0);
	СтрокаНастройки.Заголовок = НСтр("ru='Режим синхронизации'"); 
	СтрокаНастройки.Ключ = "Дин_1";
	СтрокаНастройки.ИмяБлокаИндикатора = "Дин_1_БлокРежимРаботы";
	СтрокаНастройки.Порядок = 1;
	СтрокаНастройки.ТипИндикатора = "РежимРаботы";
	СтрокаНастройки.ДополнительныеПараметры = Новый Структура;
	
	Для Счетчик = 1 По НастройкиИндикаторов.ИндикаторыДинамики.Количество() - 1 Цикл
		
		СтрокаНастройки = НастройкиИндикаторов.ИндикаторыДинамики[Счетчик];
		СтрокаНастройки.Порядок = СтрокаНастройки.Порядок + 1;
		ПрежнийКюч = СокрЛП(СтрокаНастройки.Ключ);
		СтрокаНастройки.Ключ = "Дин_" + Формат(СтрокаНастройки.Порядок, "ЧДЦ=0; ЧН=0; ЧГ=");
		СтрокаНастройки.ИмяБлокаИндикатора = СтрЗаменить(СтрокаНастройки.ИмяБлокаИндикатора, ПрежнийКюч, СтрокаНастройки.Ключ);
...
Процедура ОбновитьАктуальныеНастройкиИндикаторов(НастройкиИндикаторов, ОбновитьНастройкиПользователя = Истина)
	...
	НачалоПериодаБюджета    = ?(ЗначениеЗаполнено(НастройкиИндикаторов.НачалоПериодаБюджета), НастройкиИндикаторов.НачалоПериодаБюджета, НачалоМесяца(ТекущаяДатаКлиентСервер()));
	КонецПериодаБюджета     = ?(ЗначениеЗаполнено(НастройкиИндикаторов.КонецПериодаБюджета), НастройкиИндикаторов.КонецПериодаБюджета, КонецМесяца(ТекущаяДатаКлиентСервер()));
...
Процедура ДобавитьРеквизитыПоНастройкамИндикаторовДинамики(МассивРеквизитов, СписокИндикаторов)
	...
	Для каждого НастройкаИндикатора Из СписокИндикаторов Цикл
		...
		ИначеЕсли ТипИндикатора = "РежимРаботы" Тогда
...
Процедура ПрименитьНастройкиКИндикаторамСобытий(НастройкиОбщие, СтрокаНастроек)
	...
	Если СтрокаНастроек.ТипИндикатора = "РежимРаботы" Тогда
		ПрименитьНастройкиИндикатораРежимРаботы(НастройкиОбщие, СтрокаНастроек);
	ИначеЕсли СтрокаНастроек.ТипИндикатора = "ВводОпераций" Тогда
...
Процедура ВыполнитьЭтапОбновленияДанных()
	...
	ИначеЕсли ЭтапОбновленияДанных = "Старт" Тогда
		...
		ДлительнаяОперация = НачатьОбновлениеДанныхНаСервере();
		...
		ИначеЕсли ДлительнаяОперация.Статус = "Выполнено" Тогда
			ЗавершитьОбновлениеДанныхНаСервере(ДлительнаяОперация, Неопределено);
			
		Иначе
			ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
			ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
			ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗавершитьОбновлениеДанныхНаСервере", ЭтотОбъект);
...
&НаСервере
Функция НачатьОбновлениеДанныхНаСервере()
	
	ОтменитьОбновлениеДанныхНаСервере();
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.ОжидатьЗавершение = 0;
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Обновление начальной страницы'");
	...
	Результат = ДлительныеОперации.ВыполнитьВФоне("Обработки.НачальнаяСтраница.ПодготовитьДанные",
		СтруктураПараметров, ПараметрыВыполнения);
		
	ИДФоновогоЗадания = ?(Результат = Неопределено, Неопределено, Результат.ИдентификаторЗадания);
	Возврат Результат;
	
КонецФункции

&НаКлиенте
Процедура ЗавершитьОбновлениеДанныхНаСервере(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		АдресФоновогоЗадания    = Результат.АдресРезультата;
	...
	ПодключитьОбработчикОжидания("ВыполнитьЭтапОбновленияДанных", 0.5, Истина);
...
Процедура ОбновитьДанныеСПроверкойДоступностиВвода()
	...
	Если (Не ОткрытиеПриложенияОбработано Или ВводДоступен()) И Не ЗначениеЗаполнено(ИДФоновогоЗадания) И Не НеОбновлятьАвтоматически Тогда
		ОткрытиеПриложенияОбработано = Истина;
		ВыполнитьЭтапОбновленияДанных();
	Иначе
...
Процедура ИзменитьПодзаголовокФормы(Форма, СостояниеОбновления = 0)
	...
	Иначе
		...
		ТекстПодзаголовка = СтрШаблон(ТекстПодзаголовка, Формат(ТекущаяДатаКлиентСервер(), "Л=ru_RU; ДЛФ=T"));
		ТекстПодзаголовка = Новый ФорматированнаяСтрока(
...
Процедура ОбновитьИндикаторыСобытий(СтруктураВыборок, НастройкиИндикаторов)
	...
	Для каждого СтрокаНастройки Из СписокИндикаторов Цикл
		...
		Если СтрокаНастройки.ТипИндикатора = "РежимРаботы" Тогда
			
			ОбновитьИндикаторыРежимРаботы(СтрокаНастройки, ВыборкаИндикатора, НастройкиИндикаторов);
			
		ИначеЕсли СтрокаНастройки.ТипИндикатора = "ВводОпераций" Тогда
...
#Область ИндикаторРежимаРаботы


&НаСервере
Процедура ПрименитьНастройкиИндикатораРежимРаботы(НастройкиОбщие, СтрокаНастроек)
	...
	Элементы[Префикс + "РежимРаботыЗаголовок"].Заголовок = ТекстЗаголовка;

КонецПроцедуры


&НаСервере
Процедура ОбновитьИндикаторыРежимРаботы(СтрокаНастройки, СтруктураРезультата, НастройкиИндикаторов)
	...
	Модуль.ОбновитьИндикаторРежимаРаботыНаНачальнойСтранице(Элементы, СтрокаНастройки, 
			СтруктураРезультата, НастройкиИндикаторов, ЦветЗаголовкаИндикатора());
...
Процедура ПрименитьНастройкиИндикатораВводОпераций(НастройкиОбщие, СтрокаНастроек)
	...
	Если Не ЗначениеЗаполнено(СтрокаНастроек.Период.ДатаНачала) Тогда
		СтрокаНастроек.Период.ДатаНачала    = НачалоДня(ТекущаяДатаКлиентСервер());
...
Процедура ОбновитьИндикаторыСтруктуры(СтрокаНастройки, РезультатЗапросаСтруктуры, РезультатЗапросаПредПериода, НастройкиИндикаторов)
	...
	ТочкаДиаграммы = Диаграмма.УстановитьТочку(?(СтрокаНастройки.ВидОбъектовУчета = "Доходы", "Доход", "Расход"));
...
Процедура ОбновитьИндикаторыДинамики(СтрокаНастройки, РезультатЗапроса, НастройкиИндикаторов)
	...
	ИмяГруппировкиПериода = ОбщегоНазначенияДеньги.ИмяГруппировкиПоДлительности(СтрокаНастройки.Период.ДатаНачала, 
				Мин(НачалоДня(ТекущаяДатаКлиентСервер()), СтрокаНастройки.Период.ДатаОкончания));
...
Процедура ОбновитьИндикаторыКурсовВалют(СтрокаНастройки, РезультатЗапроса, НастройкиИндикаторов)
	...
	МинКурс   = 9999999;
	Счетчик = 0;
	Завтра  = КонецДня(ТекущаяДатаКлиентСервер()) + 1;
	...
	Пока ВыборкаПоВалютам.Следующий() Цикл
		...
		Элементы[ПрефиксПоля + "КурсыВалютЛегенда" + Счетчик].ЦветФона = Диаграмма.ПолучитьПалитру()[Диаграмма.Серии.Индекс(СерияВалюты)];
...
Процедура ОбновитьИндикаторНакоплений(СтрокаНастройки, РезультатЗапроса, НастройкиИндикаторов, ТаблицаНакоплений)
	...
	Для каждого СтрокаТаблицы Из ТаблицаНакоплений Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.Срок) Тогда
			...
			Если СтрокаТаблицы.Срок > ТекущаяДатаКлиентСервер() Тогда
...
Процедура ОбновитьИндикаторБюджета(СтрокаНастройки, РезультатЗапроса, НастройкиИндикаторов)
	...
	ОтНачалаПериода = НачалоДня(ТекущаяДатаКлиентСервер()) - НастройкиИндикаторов.НачалоПериодаБюджета;
	...
	ТекстПодсказки = Формат(ТекущаяДатаКлиентСервер(), "ДФ='д ММММ, дддд'");
...
Функция ЗаполнитьИндикаторОстаткаВыборкой(СтрокаНастройки, РезультатЗапроса, ДополнительныеДанные, СуммыСКопейками)
	...
	// Получаем суммы
	Сумма      = ?(ЗначениеЗаполнено(Выборка.АктивныйНаКонец), Окр(Выборка.АктивныйНаКонец, ?(СуммыСКопейками, 2, 0)), 0);
...
Функция ПредставлениеПериодаВЗаголовкеОстатков(СтруктураПериода)
	...
	Иначе
		...
		Если Год(СтруктураПериода.ДатаНачала) = Год(ТекущаяДатаКлиентСервер()) Тогда
...
Функция ПредставлениеДатыВПериодеС(Знач ДатаНачала)
	...
	ТекущаяДата = НачалоДня(ТекущаяДатаКлиентСервер());
...
Функция ПредставлениеДатыВПериодеПо(Знач ДатаОкончания)
	...
	ТекущаяДата = КонецДня(ТекущаяДатаКлиентСервер());
...
Функция СокращенноеПредставлениеДаты(Дата, СокращатьОтдаленныеСроки = Ложь)
	...
	Сегодня = НачалоДня(ТекущаяДатаКлиентСервер());
...
Процедура ОбновитьСтруктуруПериода(СтруктураПериода, ВидПериода, НачалоБюджетногоПериода, ОкончаниеБюджетногоПериода)
	...
	ИначеЕсли Найти("НачалоПроизвольнойДаты,КонецПроизвольнойДаты", ВидПериода) > 0 
		...
		ДатаНачала    = ?(ЗначениеЗаполнено(СтруктураПериода.ДатаНачала), СтруктураПериода.ДатаНачала, НачалоДня(ТекущаяДатаКлиентСервер()));
...
Функция НоваяСтруктураПериода(ВидПериода, НачалоПериода = Неопределено, КонецПериода = Неопределено)
	...
	ИначеЕсли ВидПериода = "НачалоПроизвольнойДаты" Или ВидПериода = "КонецПроизвольнойДаты" Тогда
		...
		Результат.ДатаНачала = ?(ЗначениеЗаполнено(Результат.ДатаНачала), Результат.ДатаНачала, НачалоМесяца(ТекущаяДатаКлиентСервер()));
	...
	ИначеЕсли ВидПериода = "ПоследняяНеделя" Тогда
		...
		Результат.ДатаОкончания = КонецДня(ТекущаяДатаКлиентСервер());
...
Процедура ОбновитьНастройкуИндикатораДинамикиСервер(СтруктураНастройки)
	...
	Если СтруктураНастройки.ТипИндикатора <> "РежимРаботы" Тогда
...
&НаСервереБезКонтекста
Процедура ВключитьОбменНаСервере(СОблаком)
	...
	Константы.ДеньгиВыбранныйСпособСинхронизации.Установить(
		?(СОблаком, Перечисления.СпособыСинхронизацииДанных.СинхронизацияССервисом, 
...
&НаКлиенте
Процедура ОткрытьФормуПодсистемыСинхронизацииСЦБ(ИмяФормы)
