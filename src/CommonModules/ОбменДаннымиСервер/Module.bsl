...
// Конец ДЕНЬГИ
	...
	// ДЕНЬГИ
	Если ВидТранспорта = Перечисления.ВидыТранспортаСообщенийОбмена.Dropbox
		Или ВидТранспорта = Перечисления.ВидыТранспортаСообщенийОбмена.GoogleDrive
		Или ВидТранспорта = Перечисления.ВидыТранспортаСообщенийОбмена.ЯндексДиск Тогда
		
		ИмяВидаТранспорта = ОбщегоНазначения.ИмяЗначенияПеречисления(Перечисления.ВидыТранспортаСообщенийОбмена.FILE);
	Иначе
		ИмяВидаТранспорта = ОбщегоНазначения.ИмяЗначенияПеречисления(ВидТранспорта);
	КонецЕсли;
	
	Возврат СтрЗаменить( "ТранспортСообщенийОбмена[ВидТранспорта]", "[ВидТранспорта]", ИмяВидаТранспорта );
	// Конец ДЕНЬГИ
...
Процедура ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Отказ, НастройкаВыполненияОбмена, НомерСтроки = Неопределено) Экспорт
	...
	// ДЕНЬГИ
	СтандартнаяОбработка = Истина;
	СинхронизацияАвтономныхКопий.ОбработкаОбменаДаннымиПоСценарию(НастройкаВыполненияОбмена, НомерСтроки, СтандартнаяОбработка);
