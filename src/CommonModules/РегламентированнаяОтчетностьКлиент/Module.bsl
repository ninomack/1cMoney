...
Процедура ВыбратьФормуОтчетаИзДействующегоСпискаЗавершение(ВыбСтрока, ДополнительныеПараметры) Экспорт
	...
	Если Не ВыбСтрока = Неопределено Тогда
		...
		// ДЕНЬГИ
		Форма.ПолеРедакцияФормы	= ВыбСтрока[0].РедакцияФормы;
		// Конец ДЕНЬГИ
	...
	Если ТипЗнч(Форма) = Тип("УправляемаяФорма") Тогда
		...
		Если СтрНачинаетсяС(Форма.ИмяФормы, "Отчет.РегламентированноеУведомление") Тогда
			...
			Если Форма.Объект.Ссылка.Пустая() Тогда
				...
				ДопПараметры.Вставить("Форма", Форма);
				ДопПараметры.Вставить("ТипВладельцаФайла", Тип("ДокументСсылка.РегламентированныйОтчет"));
				// ДЕНЬГИ Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения"));
...
Процедура ОткрытьФормуПрисоединенныеФайлы(Результат, ДопПараметры) Экспорт
	...
	// Конец ДЕНЬГИ	
		...
		ИмяФормыФайлов = "Обработка.РаботаСФайлами.Форма.ПрисоединенныеФайлы"; // ДЕНЬГИ
		ОткрытьФорму(ИмяФормыФайлов, // ДЕНЬГИ
...
Процедура ПоказатьПодтверждениеЗакрытияФормы(Знач ОповещениеСохранитьИЗакрыть, Отказ, 
	Знач ЗавершениеРаботы, Знач ТекстПредупреждения = "", ТекстПредупрежденияПриЗавершении = Неопределено, Форма = Неопределено) Экспорт
...
Процедура ВывестиМашиночитаемуюФормуВТонкомКлиентеЗавершение(ВремКаталог, ВремФайл, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, Знач ИмяКнопки, ТаксДокПринт, Форма)
	...
	Если ТаксДокПринт <> Неопределено Тогда
		...
		Исключение
			...
			ОбработатьОшибкуМодуляПечати(Ошибка.Описание, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ВремКаталог);
			ТаксДокПринт = Неопределено;
		КонецПопытки;
	КонецЕсли;
...
Процедура ИзменитьСтатусОтправкиПослеВыбораИзСпискаЗавершение(РезультатВопроса, ПараметрыИзменения) Экспорт
	...
	Если НЕ ПараметрыИзменения.Свойство("КонтролирующийОрган", КонтролирующийОрган) Тогда 
		// ДЕНЬГИ  ИЛИ КонтролирующийОрган <> ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.Минобороны") Тогда
		Форма.Модифицированность = Истина;
	КонецЕсли;
