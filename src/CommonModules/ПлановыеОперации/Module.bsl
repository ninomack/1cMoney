	...
	Если Объект.ЭтоШаблон Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "АктивностьШаблона") 
			...
			Если Форма.ПараметрыРасписания.НеИспользовать И 
					(Не ЗначениеЗаполнено(Форма.ПараметрыРасписания.ДатаОкончанияРасписания) ИЛИ Форма.ПараметрыРасписания.ДатаОкончанияРасписания > ТекущаяДатаСеанса()) Тогда
				Форма.ПараметрыРасписания.ДатаОкончанияРасписания = ТекущаяДатаСеанса();
			ИначеЕсли Не Форма.ПараметрыРасписания.НеИспользовать И Форма.ПараметрыРасписания.ДатаОкончанияРасписания < ТекущаяДатаСеанса() Тогда
...
Процедура СформироватьОборотыШаблонаДокументов(СсылкаНаШаблон, ДополнительныеСвойства = Неопределено) Экспорт
	...
	Пока ВыборкаДат.Следующий() Цикл
		...
		Если ВыборкаДат.ПлановаяОперация = СсылкаНаШаблон И СсылкаНаШаблон.ПометкаУдаления
			И Не ВыборкаДат.Пропустить И ЗначениеЗаполнено(ВыборкаДат.ПлановаяДата) Тогда
			
			// Изменение пометки удаления у шаблона
			ЗаписьДаты = РегистрыСведений.ДатыРасписаний.СоздатьМенеджерЗаписи();
			ЗаписьДаты.ВладелецРасписания = СсылкаНаШаблон;
			ЗаписьДаты.ПлановаяДата = ВыборкаДат.ПлановаяДата;
			ЗаписьДаты.Прочитать();
			ЗаписьДаты.Пропустить = СсылкаНаШаблон.ПометкаУдаления;
...
// Очистка ссылок в ресурсах/реквизитах регистров и в связанных реквизитах других объектов
Процедура ПередУдалениемПлановойОперации(Объект, Отказ) Экспорт
	
	ТипСсылки = ТипЗнч(Объект.Ссылка);
	Если Метаданные.РегистрыСведений.ДатыРасписаний.Ресурсы.ПлановаяОперация.Тип.СодержитТип(ТипСсылки) Тогда
		ОчиститьСсылкиНаПлановуюОперацию(Объект.Ссылка, Отказ);
	КонецЕсли;
	
КонецПроцедуры

// Удаляет даты расписаний, в которых указанные операции выступают и шаблоном и плановой операцией
//Используется в случае, когда операцию, запланированную без шаблона сделали плановой операцией шаблона
//
//Параметры:
//	СписокОпераций - Массив ссылок на документы
//
Процедура УдалитьДатуОперацийЗапланированныхБезШаблона(СписокОпераций) Экспорт
	...
	|	РегистрСведений.ДатыРасписаний КАК ДатыРасписаний
	|ГДЕ
	|	ДатыРасписаний.ВладелецРасписания В (&СписокОпераций)
	|	И ДатыРасписаний.ВладелецРасписания = ДатыРасписаний.ПлановаяОперация";
	...
	Пока Выборка.Следующий() Цикл
		Если ЗначениеЗаполнено(Выборка.ВладелецРасписания) Тогда
			...
			Запись.Операция = Выборка.ВладелецРасписания;
			Запись.Удалить();
...
Процедура ВычислитьИЗаписатьДатыРасписания(ВладелецРасписания, ПараметрыРасписания, СписокДатРасписания, ДатаПределаПланирования = Неопределено, МинимальнаяДатаИзменений = Неопределено)
	...
	Если ПараметрыРасписания.НеИспользовать Тогда
		ДатаПрекращения = ?(ЗначениеЗаполнено(КонечнаяДата), КонечнаяДата, НачалоДня(ТекущаяДатаСеанса()));
	Иначе
...
Процедура ОчиститьСсылкиНаПлановуюОперацию(Ссылка, Отказ) 
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ПлановаяОперация", Ссылка);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ДатыРасписаний.*
	|ИЗ
	...
	|ГДЕ
	|	ФинансовыеЦели.ШаблонНакопления = &ПлановаяОперация
	|
	|";
	...
	Пока Выборка.Следующий() Цикл
		ЗаписьРегистра = РегистрыСведений.ДатыРасписаний.СоздатьМенеджерЗаписи();
		ЗаполнитьЗначенияСвойств(ЗаписьРегистра, Выборка, , "ПлановаяОперация, СуммаДохода, СуммаРасхода");
		ЗаписьРегистра.Записать(Истина);
	КонецЦикла;
