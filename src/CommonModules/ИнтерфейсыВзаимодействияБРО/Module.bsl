...
Функция СоответствиеЗаменяемыхКодовОргановФСГС(Знач ОпрашиватьСервис = Ложь) Экспорт
	...
	// ДЕНЬГИ
	//Удален код, вызывающий другие подистемы
	// Конец ДЕНЬГИ	
...
Функция СвойстваОтчета(Документ) Экспорт
	...
	Результат = Новый Структура; // ДЕНЬГИ ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ПараметрыОтображенияВЖурналеОтчетов();
...
//  Требование       - Строка, имя требования (может содержать точку - ".")
//  Организация      - СправочникСсылка.Организации
//  Конец            - Дата окончания периода (без времени) 
//
// Возвращаемое значение - 
//  Неопределено,
...
// Параметры:
//  Организация      - СправочникСсылка.Организации
//  НачалоПериода    - Дата начала периода (без времени) 
//  КонецПериода     - Дата окончания периода (без времени) 
//
// Возвращаемое значение - 
//  Истина - Организация должно платить торговый сбор в периоде (есть соответствующие уведомления),
//  Ложь - иначе
...
// ДЕНЬГИ (добавлено для совместимости)
...
Функция БлижайшийНалоговыйПериод(Организация, Знач ПроверяемыйПериод, ВариантРасширенногоПервогоПериода,
	...
	Если ЗначениеЗаполнено(ПроверяемыйПериод) Тогда
		Результат.Период = ПроверяемыйПериод;
	Иначе
		Результат.Период = НачалоДня(ТекущаяДата());
	КонецЕсли;
	
	Результат.Начало = НачалоГода(Результат.Период);
	Результат.Конец  = КонецГода(Результат.Период);
...
Функция ЗаписатьРеглОтчетРСВ1ФормаОтчета2010Кв4(СтруктураПараметров, СсылкаНаСохрРеглОтчет = Неопределено)
	...
	Попытка
		...
		// ДЕНЬГИ
		// Замена неиспользуемого функционала БРО. Закомментированы строки
		//Если ДанныеРеглОтчета.НомерПачки > НомерПачкиИзРегистра Тогда
		//	РегламентированнаяОтчетность.УстановитьНомерПачкиВыгруженныхФайловПФР(Организация, Число(РазделыОтчета.Титульный.Данные.ОтчетГод), ДанныеРеглОтчета.НомерПачки);
		//КонецЕсли;
		// Конец ДЕНЬГИ
