...
Процедура КопироватьКурсыВалюты(Знач КодВалюты) Экспорт
	...
	// ДЕНЬГИ
	днгВалютаРубль = Справочники.Валюты.НайтиПоКоду("643");
	ТаблицаКурсов.Колонки.Добавить("БазоваяВалюта");
	ТаблицаКурсов.ЗаполнитьЗначения(днгВалютаРубль, "БазоваяВалюта");
	НаборЗаписей.Отбор.БазоваяВалюта.Установить(днгВалютаРубль);
	// Конец ДЕНЬГИ
	...
	РаботаСКурсамиВалют.ПроверитьКорректностьКурсаНа01_01_1980(ВалютаСсылка, Константы.ВалютаУчета.Получить());
	// Конец ДЕНЬГИ
...
Процедура ОбработатьКурсыОбластиВТранзакции(ЗапросОбщий, ВалютыОбластей, ТаблицаКурсов)
	...
	// ДЕНЬГИ
	днгВалютаРубль       = Справочники.Валюты.НайтиПоКоду("643");
	днгСостояниеЗагрузки = Новый Массив;
	днгНачальнаяДата     = ТекущаяДата();
	днгКонечнаяДата      = днгНачальнаяДата;
	// Конец ДЕНЬГИ
	...
	Пока ВыборкаВалюты.Следующий() Цикл
		...
		// ДЕНЬГИ
		ЗапросОбщий.УстановитьПараметр("ВалютаРубль", днгВалютаРубль);
		днгКусрВалютыОбновлен = Ложь;
		// Конец ДЕНЬГИ
		...
		// ДЕНЬГИ
		|		И КурсыВалют.БазоваяВалюта = &ВалютаРубль
		// Конец ДЕНЬГИ
		...
		Пока ВыборкаОбщий.Следующий() Цикл
			...
			Если ВыборкаОбщий.ВФайле = 1 Тогда
				...
				// ДЕНЬГИ
				Запись.БазоваяВалюта = днгВалютаРубль;
				Если ВыборкаОбщий.Дата < днгНачальнаяДата Тогда
					днгНачальнаяДата = ВыборкаОбщий.Дата;
				КонецЕсли;
				Если ВыборкаОбщий.Дата > днгКонечнаяДата Тогда
					днгКонечнаяДата = ВыборкаОбщий.Дата;
				КонецЕсли;
				// Конец ДЕНЬГИ
		...
		РаботаСКурсамиВалют.ПроверитьКорректностьКурсаНа01_01_1980(ВыборкаВалюты.Ссылка, Константы.ВалютаУчета.Получить());
		// Конец ДЕНЬГИ
		
		// ДЕНЬГИ
		Если днгКусрВалютыОбновлен Тогда
			днгСостояниеЗагрузки.Добавить(Новый Структура("Валюта,СтатусОперации", ВыборкаВалюты.Ссылка, Истина));
		КонецЕсли;
		// Конец ДЕНЬГИ
	...
	// ДЕНЬГИ
	РаботаСКурсамиВалют.ПересчитатьЗагруженныеКурсыВалют(днгСостояниеЗагрузки, Неопределено, днгНачальнаяДата, днгКонечнаяДата);
	// Конец ДЕНЬГИ
