...
// Возращает значение параметра запуска приложения, соответствующее режиму тестирования
//
//Параметры:
//	Нет
//
//Возвращаемое значение:
//	Стока - "testmanager", "testclient" или ""
//
Функция ТестовыйРежим() Экспорт
	
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	ПараметрЗапускаПриложения = НРег(ПараметрыСеанса.ПараметрыКлиентаНаСервере.Получить("ПараметрЗапуска"));
#Иначе
	ПараметрЗапускаПриложения = НРег(ПараметрЗапуска);
#КонецЕсли
	
	Позиция = СтрНайти(ПараметрЗапускаПриложения, "testmode");
	Если Позиция = 0 Тогда
		Возврат "";
	КонецЕсли;
	
	Режим = Сред(ПараметрЗапускаПриложения, Позиция + 9);
	Режим = Сред(Режим, 1, СтрНайти(Режим, ";") - 1);
	Возврат СтрЗаменить(Режим, "=", "");
...
Процедура ОбновитьДатыПоВидуПериода(ВидПериода, ДатаНачала, ДатаОкончания, 
	...
	НачалоСегодня = НачалоДня(ДеньгиКлиентСервер.ЗначениеТекущейДаты());
	...
	ИначеЕсли ВидПериода = "ПоследнийМесяц" Тогда
		...
		ДатаНачала     = ДобавитьМесяц(НачалоСегодня, -1);
		ДатаОкончания  = КонецСегодня;
...
Функция ЗначениеТекущейДаты() Экспорт
...
Функция ДлительностьСтрокой(ДлительностьВыполнения) Экспорт
...
//
//Параметры:
//	ПолныйПутьКаталогаИлиФайла - Строка
//
//Возвращаемое значение:
//	Булево - Истина, если файл существует
//
Функция ФайлСуществует(ПолныйПутьКаталогаИлиФайла) Экспорт
	
	Файл = Новый Файл(ПолныйПутьКаталогаИлиФайла);
