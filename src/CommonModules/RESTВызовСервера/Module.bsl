...
// Серверная обертка для выполнения запроса из веб-клиента
//
//Параметры:
//	см. параметры в RESTКлиентСервер.ВыполнитьHTTPЗапрос()
//
...
Функция ВыполнитьHTTPЗапрос(Метод, Сервер, Адрес, ТекстЗапроса, ФайлТела = "", Заголовки, Защищенный = Истина, ИмяВыходногоФайла = "", 
	...
	Возврат RESTКлиентСервер.ВыполнитьHTTPЗапрос(Метод, Сервер, Адрес, ТекстЗапроса, ФайлТела, 
...
//
//Параметры:
//	см. параметры в RESTКлиентСервер.РезультатHTTPЗапроса()
//
//Возвращаемое значение:
//	Структура
//
Функция РезультатHTTPЗапроса(Метод, СтруктураЗапроса, ТекстЗапроса, ИмяСобытия = "", ФайлТела = "", КодыУспеха = "200", ИмяВыходногоФайла = "") Экспорт

	Возврат RESTКлиентСервер.РезультатHTTPЗапроса(Метод, СтруктураЗапроса, ТекстЗапроса, ИмяСобытия, КодыУспеха, ФайлТела, ИмяВыходногоФайла);
...
Процедура ОтметитьОшибкуВЖурналеРегистрации(СтруктураОтвета, ОбъектМетаданных = Неопределено, Данные = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрации(СтруктураОтвета.ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, ОбъектМетаданных, Данные, 
				?(СтруктураОтвета.КодСостояния > 0, 
							СтрШаблон(НСтр("ru='REST-запрос вернул ответ с кодом %1:%2%3'"), 
								Строка(СтруктураОтвета.КодСостояния), Символы.ПС, СтруктураОтвета.ОписаниеОшибки)
							, ""));
