...
Процедура МобильноеПриложениеЗарегистрироватьИзменениеДокументаПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если Режим = "Разделенный" Или Режим = "Неразделенный"
		Или Источник.ДополнительныеСвойства.Свойство("ОтключитьМеханизмРегистрацииОбъектов") 
	...
	ИначеЕсли Режим = "ЦентрМобильных" Тогда 
		ОбменДаннымиСобытия.МеханизмРегистрацииОбъектовПередЗаписьюДокумента("МобильноеПриложение", Источник, Отказ, РежимЗаписи, РежимПроведения);
	КонецЕсли; 
...
Процедура МобильноеПриложениеЗарегистрироватьИзменениеКонстантыПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Режим = "Разделенный" Или Режим = "Неразделенный"
		...
		Источник.ОбменДанными.Получатели.Очистить();
	ИначеЕсли Режим = "ЦентрМобильных" Тогда 
		ОбменДаннымиСобытия.МеханизмРегистрацииОбъектовПередЗаписьюКонстанты("МобильноеПриложение", Источник, Отказ);
	КонецЕсли; 
...
Процедура МобильноеПриложениеЗарегистрироватьИзменениеРегистраПередЗаписью(Источник, Отказ, Замещение) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("ОтложенноеПроведение") Тогда
		// Записи перенесены, регистрировать нечего
		Возврат;
	КонецЕсли;
	...
	Если Источник.ДополнительныеСвойства.Свойство("ОтключитьМеханизмРегистрацииОбъектов") 
		...
		Источник.ОбменДанными.Получатели.Очистить();
	ИначеЕсли Режим = "ЦентрМобильных" Тогда 
		ОбменДаннымиСобытия.МеханизмРегистрацииОбъектовПередЗаписьюРегистра("МобильноеПриложение", Источник, Отказ, Замещение);
	КонецЕсли; 
...
////////////////////////////////////////////////////////////////////////////////
// * Настройки и параметры обмена

// Проверяет, используется ли обмен с мобильными устройствами
//
//Параметры:
//	нет
//
//Возвращаемое значение:
//	Булево - Истина, если обмен с устройствами включем
//
Функция ОбменСМобильнымиУстройствамиВключен() Экспорт

	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") 
		И Константы.СинхронизацияАвтономныхКопийЭтоЦентральнаяБаза.Получить() = Истина Тогда
        ...
        Если ПараметрыДомашняяСеть <> Неопределено И Не ПараметрыДомашняяСеть.ЗапретитьИспользование Тогда
			Возврат Истина;
		КонецЕсли;
		
		ПараметрыОблака = Справочники.ТранспортыОбменаДанными.СтруктураПараметровТранспорта(Справочники.ТранспортыОбменаДанными.ОсновнойТранспортОблачногоПровайдера(Неопределено), Истина);
        Если ПараметрыОблака <> Неопределено И Не ПараметрыОблака.ЗапретитьИспользование Тогда
			Возврат Истина;
		КонецЕсли;
...
// Возвращает количество узлоа обмена с мобильным приложением. Активность и пометки удаления не учитываются
//
//Параметры:
//	нет
//
//Возвращаемое значение:
//	число - кол-во узлов кроме ЭтотУзел. 
//
Функция КоличествоМобильныхПриложений() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ План.Ссылка) КАК КоличествоУзлов
	|ИЗ
	|	ПланОбмена.МобильноеПриложение КАК План
	|ГДЕ
	|	НЕ План.ЭтотУзел";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат ?(ЗначениеЗаполнено(Выборка.КоличествоУзлов), Выборка.КоличествоУзлов, 0);
	КонецЕсли;
...
Процедура СформироватьПакетОбмена(УзелИнформационнойБазы, Отказ, ИмяФайлаСообщения,
	...
	Если ЗначениеЗаполнено(УзелИнформационнойБазы) И ТипЗнч(УзелИнформационнойБазы) = Тип("ПланОбменаСсылка.МобильноеПриложение") И Не УзелИнформационнойБазы.Активность Тогда
		...
		Отказ = Истина;
		Ошибка = НСтр("ru = 'Обмен с мобильным устройством запрещен'");
		СинхронизацияАвтономныхКопий.ОтметитьРезультатВыгрузки(УзелИнформационнойБазы, , Ошибка);
		Возврат;
		
	КонецЕсли;
	
	СинхронизацияАвтономныхКопий.ОтметитьТекущееДействие(УзелИнформационнойБазы, НСтр("ru='Формирование пакета для отправки'") );
	...
	Попытка
		...
		Если ПараметрыОбмена.СвойстваУзлаКорреспондента.КоличествоВыгружаемыхПериодов <> 0 Тогда
			...
			СинхронизацияАвтономныхКопий.ОтметитьТекущееДействие(УзелИнформационнойБазы, НСтр("ru='Выгрузка остатков на начало'") );
	...
	Исключение
		...
		Отказ = Истина;
		Ошибка = ИнформацияОбОшибке();
		СинхронизацияАвтономныхКопий.ОтметитьРезультатВыгрузки(УзелИнформационнойБазы, КоличествоОбъектовОбработано, Ошибка);
		Возврат;
		
	КонецПопытки;
	
	СинхронизацияАвтономныхКопий.ОтметитьРезультатВыгрузки(УзелИнформационнойБазы, КоличествоОбъектовОбработано);
...
Процедура ВыполнитьВыгрузкуПакетаВФайл(УзелКорреспондента, ИмяФайла, Отказ = Ложь, ОписаниеПроблемы = "", Транспорт = Неопределено) Экспорт
	...
	СинхронизацияАвтономныхКопий.ОтметитьТекущееДействие(УзелКорреспондента, НСтр("ru='Выгрузка изменений'") );
	...
	Исключение
		...
		Отказ = Истина;
		Ошибка = ИнформацияОбОшибке();
		СинхронизацияАвтономныхКопий.ОтметитьРезультатЗагрузки(УзелКорреспондента, , Ошибка);
		Возврат;
		
	КонецПопытки; 
	
	СинхронизацияАвтономныхКопий.ОтметитьРезультатВыгрузки(УзелКорреспондента, 0);
...
Процедура ПринятьПакетОбмена(Отправитель, Отказ, ИмяФайлаСообщения = "",
	...
	СинхронизацияАвтономныхКопий.ОтметитьТекущееДействие(Отправитель, НСтр("ru='Чтение полученного пакета'"));
	...
	Если ЗначениеЗаполнено(Отправитель) И ТипЗнч(Отправитель) = Тип("ПланОбменаСсылка.МобильноеПриложение") И Не Отправитель.Активность Тогда
		...
		СинхронизацияАвтономныхКопий.ОтметитьРезультатВыгрузки(Отправитель, , ОписаниеПроблемы);
		Отказ = Истина;
		Возврат;
	...
	Исключение
		...
		РезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Ошибка;
		Ошибка = ИнформацияОбОшибке();
		СинхронизацияАвтономныхКопий.ОтметитьРезультатЗагрузки(Отправитель, , Ошибка);
		Отказ = Истина;
		Возврат;
	...
	Иначе
		...
		РезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Ошибка;
		Ошибка = НСтр("ru = 'В сообщении обмена нет настроек (Settings)'");
		СинхронизацияАвтономныхКопий.ОтметитьРезультатЗагрузки(Отправитель, , Ошибка);
		Отказ = Истина;
		Возврат;
	...
	Если ВерсияОбмена < ОбменМобильноеПриложениеВызовСервера.МинимальноДопустимаяВерсияОбмена() Тогда
		...
		РезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Ошибка;
		Ошибка = НСтр("ru = 'Для выполнения синхронизации нужно обновить мобильное приложение'");
		СинхронизацияАвтономныхКопий.ОтметитьРезультатЗагрузки(Отправитель, , Ошибка);
		Отказ = Истина;
		Возврат;
	...
	ИначеЕсли ВерсияОбмена > ОбменМобильноеПриложениеВызовСервера.НомерВерсииОбмена() Тогда
		...
		РезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Ошибка;
		Ошибка = НСтр("ru = 'Для выполнения синхронизации нужно обновить настольное приложение'");
		СинхронизацияАвтономныхКопий.ОтметитьРезультатЗагрузки(Отправитель, , Ошибка);
		Отказ = Истина;
		Возврат;
	...
	Попытка
		...
		Если Сценарий = Перечисления.СценарииОбменаСМобильнымПриложением.НастольныеЗаменитьМобильными Тогда
			...
			Если Не ОбщегоНазначенияДеньги.ИнформационнаяБазаПустая() Тогда
				...
				РезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Ошибка;
				Отказ = Истина;
				Ошибка = НСтр("ru='Настольная программа не пустая. Заменить ее данные данными мобильного приложения невозможно.'"); 
				СинхронизацияАвтономныхКопий.ОтметитьРезультатЗагрузки(Отправитель, , Ошибка);
				Возврат;
	...
	ПараметрыОбмена.ВремяОкончанияОбмена = ТекущаяДатаСеанса();
	СинхронизацияАвтономныхКопий.ОтметитьРезультатЗагрузки(Отправитель, КоличествоПолученныхОбъектов);
...
Процедура ВыполнитьЧтениеПакетаИзФайла(ИмяФайла, УзелКорреспондента, Отказ = Ложь, ОписаниеПроблемы = "", Транспорт = Неопределено) Экспорт
	...
	Попытка
		...
		// Проверяем разрешение узла на выполнение обмена
		Если ЗначениеЗаполнено(УзелКорреспондента) 
				И ТипЗнч(УзелКорреспондента) = Тип("ПланОбменаСсылка.МобильноеПриложение") 
				И Не УзелКорреспондента.Активность Тогда
			ОписаниеПроблемы = НСтр("ru='Обмен с мобильным устройством запрещен'"); 
...
Процедура ВыполнитьОбменДаннымиПоСценариюТранспортаОбмена(ТранспортОбмена) Экспорт
	...
	СтруктураНастроекОбмена.ОбработкаТранспортаСообщенийОбмена.ВыполнитьДействияПослеОбработкиСообщения();
...
Функция ПолучитьОбъектXDTOИзДанных(ПараметрыОбмена, Данные, ТипОбъекта = "")
	...
	ВерсияОбмена      = ПараметрыОбмена.СвойстваУзлаКорреспондента.ВерсияОбмена;
	Если СтрНайти("Справочники,Документы,Константы,РегистрыСведений", ТипОбъекта) > 0
		И Не Метаданные.ПланыОбмена.МобильноеПриложение.Состав.Содержит(МетаданныеОбъекта) Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПолучитьОбъектXDTOСправочника(ЭлементСправочника, СоответствиеРеквизитов, СопоставленияПоСсылкам)
	...
	Если ДеньгиКлиентСервер.ИмяПеременнойВалидно(ЭлементСправочника.ИмяПредопределенныхДанных) Тогда
...
Функция ПолучитьОбъектXDTOНабораРегистраСведений(Данные, МетаданныеОбъекта) 
	...
	Если ИмяРегистра = "СоответствияОбъектовИнформационныхБаз" Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция УточнитьЗначениеДляПомещенияВОбъектXDTO(Знач ЗначениеРеквизита, СопоставленияПоСсылкам = Неопределено)
	...
	ИначеЕсли ТипРеквизита = Тип("СправочникСсылка.ГрафыБюджета") Тогда
...
Функция ПолучитьЭлементСправочникаИзXDTO(Объект, ПараметрыОбмена)
	...
	Иначе
		...
		Если Объект.Свойства().Получить("PredefinedDataName") <> Неопределено 
			...
			И ДеньгиКлиентСервер.ИмяПеременнойВалидно(Объект.PredefinedDataName) Тогда
			...
			Если НЕ ОбщегоНазначения.СсылкаСуществует(Ссылка) Тогда
				...
				Если ЕстьИерархия И Объект.Свойства().Получить("Parent") <> Неопределено
					...
					Попытка
						...
						Если ДеньгиКлиентСервер.ИмяПеременнойВалидно(ИДРодителя) Тогда
	...
	ИначеЕсли ИмяСправочника = "ГрафыБюджета" И Предопределенный Тогда
		Возврат Ссылка;
	Иначе
...
Процедура ПерепровестиЗагруженныеДокументы(ПараметрыОбмена)
	...
	Если МинимальнаяДата <> Неопределено Тогда
		ОбслуживаниеДокументов.ПерепровестиИзмененияОстатков(МинимальнаяДата, ПараметрыОбмена.УзелКорреспондента);
	КонецЕсли;
...
Функция ПараметрыЦентральнойБазыМобильныхПриложений(ОбновлятьИмяБазы = Ложь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат.ИмяЭтойБазы) Или ОбновлятьИмяБазы Тогда
		Результат.ИмяЭтойБазы = "Money_" + Формат(ТекущаяДатаСеанса(), "ДФ=дчс");
