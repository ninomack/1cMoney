...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
		"РегистрСведений.ОрганизацииЗаключившиеСоглашениеСПФР");
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	Если НЕ ОбъектМетаданных = Неопределено Тогда
		...
		Описание = Описание + "
		|РегистрСведений.ОрганизацииЗаключившиеСоглашениеСПФР.Чтение.Организации
		|РегистрСведений.ОрганизацииЗаключившиеСоглашениеСПФР.Изменение.Организации";	
...
Процедура СохранитьСтатусОтправкиРеглОтчета(Форма) Экспорт
	...
	ЭтоОтчетМинобороны = Ложь; // ДЕНЬГИ Форма.СтруктураРеквизитовФормы.мСохраненныйДок.ИсточникОтчета = "РегламентированныйОтчетИсполнениеКонтрактовГОЗ";
	// ДЕНЬГИ
...
Функция СвойстваПрочихОтчетов(Ссылка) Экспорт
	...
	ПараметрыОтчета = Новый Структура; // ДЕНЬГИ ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ПараметрыОтображенияВЖурналеОтчетов();
...
Функция СвойстваРегламентированногоОтчета(Ссылка) Экспорт
	...
	ПараметрыОтчета.Вставить("ДокСсылка",                "");
	...
	ПараметрыОтчета.Вставить("ДатаНачалаОП",             "");
	ПараметрыОтчета.Вставить("ДатаОкончанияОП",          "");
	...
	ПараметрыОтчета.Вставить("ДатаСоздания",             "");
	ПараметрыОтчета.Вставить("Комментарий",              "");
	ПараметрыОтчета.Вставить("Банк",                     "");
...
Процедура ВыполнитьДействияПередУдалениемПрисоединенногоФайла(Источник, Отказ) Экспорт
	...
	Если ТипЗнч(Источник) = Вычислить("Тип(""СправочникОбъект.ВерсииФайлов"")") Тогда // ДЕНЬГИ
		Возврат;
	КонецЕсли;
...
Функция ТаблицаОписанияОбъектовРегламентированнойОтчетности() Экспорт
	...
	// ДЕНЬГИ ЭлектронныйДокументооборотСКонтролирующимиОрганами.ОпределитьТаблицуОписанияОбъектовРегламентированнойОтчетности(ТаблицаОписания);
...
Процедура ВыполнитьПроверку(Параметры, АдресХранилища) Экспорт
	...
	КоличествоОбъектовНеЗаписанныхВРегистрЖурналОтправокВКонтролирующиеОрганы = 0; // ДЕНЬГИ 
		// ДЕНЬГИ ДокументооборотСКОВызовСервера.КоличествоОбъектовНеЗаписанныхВРегистрЖурналОтправокВКонтролирующиеОрганы();
...
Процедура ПолучитьСписокДоступныхИндивидуальныхПредпринимателей(СписокФизЛиц = Неопределено, Знач Текст = Неопределено) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
				   ...
				   |	Справочник.Организации КАК Организации
				   |ГДЕ
			// ДЕНЬГИ	   |	Организации.ЮридическоеФизическоеЛицо = ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо) И
	...
	// ДЕНЬГИ РегламентированнаяОтчетностьПереопределяемый.СформироватьТекстЗапросаДляПолученияДоступныхИндивидуальныхПредпринимателей(
	// ДЕНЬГИ	Запрос.Текст);
...
Процедура ПолучитьТаблицуПараметровИБ(ТаблицаПараметров) Экспорт
	...
	МодульРаботаВМоделиСервиса.ДобавитьКонстантуВТаблицуПараметровИБ(ТаблицаПараметров, "ИспользоватьСервисФормированияМЧБсPDF417");
	МодульРаботаВМоделиСервиса.ДобавитьКонстантуВТаблицуПараметровИБ(ТаблицаПараметров, "ИспользоватьСервисФормированияМЧБсPDF417ТолькоВРежимеВебКлиента");
	МодульРаботаВМоделиСервиса.ДобавитьКонстантуВТаблицуПараметровИБ(ТаблицаПараметров, "АдресСервисаФормированияМЧБсPDF417");
	МодульРаботаВМоделиСервиса.ДобавитьКонстантуВТаблицуПараметровИБ(ТаблицаПараметров, "ИмяПользователяСервисаФормированияМЧБсPDF417");
	МодульРаботаВМоделиСервиса.ДобавитьКонстантуВТаблицуПараметровИБ(ТаблицаПараметров, "ПарольПользователяСервисаФормированияМЧБсPDF417");
	// Конец ДЕНЬГИ	
		...
		Для НомСтроки = 1 По КоличествоСтрокВТаблице Цикл
			...
			Если Инд > 0 Тогда
				Если СтрокаТаблицы.Код  = ТаблицаОтобранных[Инд - 1].Код
				   И СтрокаТаблицы.ОКУД = ТаблицаОтобранных[Инд - 1].ОКУД Тогда
...
// Удаленные методы
// ЗаменитьСсылкиРазделенныйСпрУдалитьРеглОтчетыНаНеРазделенныйСпрРеглОтчеты
// ЗаменитьРеквизитВидОтчета
// УстановитьПризнакНаличияФайловПрисоединенныхКУведомлению
