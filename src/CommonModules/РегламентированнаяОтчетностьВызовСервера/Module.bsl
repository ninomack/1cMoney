...
Функция ПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Если Организация = Неопределено ИЛИ Организация = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа("СправочникСсылка.Организации") Тогда
		...
		Возврат ДеньгиКлиентСервер.ПолучитьСтруктуруОрганизации();
		// Конец ДЕНЬГИ
	КонецЕсли;
...
Функция ПолучитьСведенияОПредставителе(ФормаОрганизация, ФормаДатаПодписи, Знач МожетБытьТолькоЮРЛицо = Неопределено, Знач ИФНС = Неопределено, КПП = Неопределено) Экспорт
	...
	Если РегламентированнаяОтчетность.ПредставительЯвляетсяФизЛицом(Представитель) Тогда
		...
		// ДЕНЬГИ
		// Замена неиспользуемого функционала БРО. Вместо строк:
		...
		ДанныеКонтактнойИнформации.Вставить("ПолУпПред", 0);
		// Конец ДЕНЬГИ
		...
		//	
		//	Для НомСтр = 1 По СтрЧислоСтрок(Результат[0].ЗначенияПолей) Цикл
		//		
		//		Стр = СтрПолучитьСтроку(Результат[0].ЗначенияПолей, НомСтр);
		//		
...
Функция ИННСоответствуетТребованиямНаСервере(Знач ИНН, Знач ТипЛица) Экспорт
	...
	Иначе
		...
		ПроверяетсяИННФизЛица = (ТипЛица = Истина);
		// Конец ДЕНЬГИ
	КонецЕсли;
...
Функция ПолучитьШаблоныМашиночитаемыхФорм(Знач ИмяОтчета, ПрефиксИмениМакетаШаблона, КоличествоВыгруженныхШаблонов, ТочныеПараметрыШаблонаЗаданы, ПрефиксИмениШаблона, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм) Экспорт
	...
	Для Каждого МакетОтчета Из МакетыОтчета Цикл
		Если (МакетОтчета.ТипМакета = Метаданные.СвойстваОбъектов.ТипМакета.ДвоичныеДанные
			ИЛИ МакетОтчета.ТипМакета = Метаданные.СвойстваОбъектов.ТипМакета.ВнешняяКомпонента) // ДЕНЬГИ
...
Функция ПолучитьПоКодамСведенияОПредставителе(Организация, КодНО, КПП = Неопределено) Экспорт
	...
	// ДЕНЬГИ
	// удален код, использующий другие подсистемы
	// Конец ДЕНЬГИ
	
	Результат = Новый Структура;
	Результат.Вставить("ТипПодписанта", "");
	Результат.Вставить("флПредставительЮрЛицо", "");
	Результат.Вставить("НаименованиеОрганизацииПредставителя", СокрЛП(Организация));
	Результат.Вставить("ФИОПредставителя", "");
	Результат.Вставить("ПредставительСсылка", "");
...
Процедура ОтразитьВЖурналеОтчетов(ПараметрыОтчета) Экспорт
	...
	Попытка
		...
		ОбъектОтправляетсяВКонтролирующиеОрганы =
			Ложь; // ДЕНЬГИ = ЭлектронныйДокументооборотСКонтролирующимиОрганами.ОбъектОтправляетсяВКонтролирующиеОрганы(ДокСсылка);
		...
		Непрочтенное = Ложь; // ДЕНЬГИ ЭлектронныйДокументооборотСКонтролирующимиОрганами.ОпределитьНепрочтенность(
			//ДокСсылка,
			//"ЖурналОтчетовСтатусы",
			//СостояниеСдачиОтчетности,
			//Перечисления.СтраницыЖурналаОтчетность.Отчеты);
			
		ДатаОтправкиОтчета = ТекущаяДата(); // ДЕНЬГИ ЭлектронныйДокументооборотСКонтролирующимиОрганами.ПолучитьДатуОтправки(ДокСсылка, ВидКонтролирующегоОргана);
		...
		Запись.ЕстьКритическиеОшибкиОтправки      = Ложь; // ДЕНЬГИ ЭлектронныйДокументооборотСКонтролирующимиОрганами.ЕстьКритическиеСообщения(ДокСсылка);
...
Функция ВозможноПрисоединениеФайловКОбъекту(ВладелецФайлов) Экспорт
	...
	ИмяСправочника = МодульРаботаСФайламиСлужебный.ИмяСправочникаХраненияФайлов(ВладелецФайлов, ""); // ДЕНЬГИ
...
Функция ВыборкаОбъектовНеЗаписанныхВРегистрЖурналОтчетовСтатусы()
	...
	// ДЕНЬГИ МассивТипов.Добавить(Тип("СправочникСсылка.ЭлектронныеПредставленияРегламентированныхОтчетов"));
	...
	ОписанияОбъектовНеВходящихВБРО = ТаблицаОписанияОбъектовНеВходящихВБРО.НайтиСтроки(
		Новый Структура("ВидДокумента", 	"Отчеты")); // ДЕНЬГИ Перечисления.СтраницыЖурналаОтчетность.Отчеты));
	...
	Для каждого ТипОбъекта Из МассивТипов Цикл
		...
		Если ВРег(СокрЛП(ПолноеИмяТипа)) = ВРег("Справочник.ЭлектронныеПредставленияРегламентированныхОтчетов") Тогда
			// ДЕНЬГИ ТипОбъекта = Тип("СправочникСсылка.ЭлектронныеПредставленияРегламентированныхОтчетов") Тогда
			Запрос.Текст = Запрос.Текст + "
				|И %1.ВидОтчета НЕ В (&ВидыОтчетовРазделаУведомления)";
				
			ВидыОтчетовРазделаУведомления = Неопределено; // ДЕНЬГИ ЭлектронныйДокументооборотСКонтролирующимиОрганами.ВидыЭлектронныхПредставленийВРазделеУведомления();
...
Функция ТипСмещения(ТипСтрокой) Экспорт
	Возврат ТипСмещенияТабличногоДокумента[ТипСтрокой];
КонецФункции
