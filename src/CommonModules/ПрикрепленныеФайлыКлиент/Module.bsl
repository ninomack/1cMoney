...
Функция ВыгрузитьДанныеВФайл(Знач Адрес, Знач Расширение, Знач Наименование) Экспорт
	...
	Если Наименование = "" Тогда		
		Наименование = ПрикрепленныеФайлыКлиентСервер.АвтоНаименованиеФайла( Неопределено, ОбщегоНазначенияКлиент.ДатаСеанса() );
	КонецЕсли;
...
Функция СтруктураФайлаПоЕгоИмени(ИмяФайла, ВладелецХранилища = Неопределено)
	...
	Результат.ДатаСоздания   = ОбщегоНазначенияКлиент.ДатаСеанса();
	Результат.Размер         = ТекФайл.Размер();
	
	ИД = Неопределено; 
	Если ТипЗнч(ВладелецХранилища) = Тип("ФормаКлиентскогоПриложения") Тогда
