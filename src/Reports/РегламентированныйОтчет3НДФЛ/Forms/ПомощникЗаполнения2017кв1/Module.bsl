////////////////////////////////////////////////////////////////////////////////
// Модуль формы <ПомощникЗаполнения2017кв1>
...
Процедура ЗаполнитьЗначениямиПоУмолчанию(Форма)
	...
	Если Форма.ТаблицаУбытковПоЦеннымБумагамИИС.Количество() < 1 Тогда
		НачальныйГод   = Макс(2015, Форма.ГодОтчета - 11);
		ПредельныйГод  = Форма.ГодОтчета - 1;
		Для Счетчик = НачальныйГод По ПредельныйГод Цикл
			НоваяСтрока = Форма.ТаблицаУбытковПоЦеннымБумагамИИС.Добавить();
			НоваяСтрока.Год = Счетчик;
		КонецЦикла; 
	КонецЕсли; 
...
Процедура ЗаполнитьПоУмолчаниюДокумент2НДФЛ(ИДСтроки)
	...
	СтрокаДокумента.ШапкаДокумента.Вставить("ИндивидуальныйИнвестиционныйСчет", Ложь);
...
Процедура ЗаполнитьПоУмолчаниюДокументПрочийДоход(ИДСтроки)
	...
	СтрокаДокумента.ШапкаДокумента.Вставить("ИндивидуальныйИнвестиционныйСчет", Ложь);
	СтрокаДокумента.ШапкаДокумента.Вставить("КадастроваяСтоимость", 0);
	СтрокаДокумента.ШапкаДокумента.Вставить("ДоходБезКадастровойСтоимости", 0);
...
Процедура ЗаполнитьПоУмолчаниюДокументДоходВВалюте(ИДСтроки)
	...
	СтрокаДокумента.ШапкаДокумента.Вставить("ДатаДохода", Дата(ГодОтчета, 1, 1));
	СтрокаДокумента.ШапкаДокумента.Вставить("КодВидаДохода", 2); // 1 - прибыль контролируемых организаций, 2 - иное
	СтрокаДокумента.ШапкаДокумента.Вставить("КодДохода", "");
	СтрокаДокумента.ШапкаДокумента.Вставить("НаименованиеДохода", "");
	...
	СтрокаДокумента.ШапкаДокумента.Вставить("НомерИностраннойОрганизации", "");
	СтрокаДокумента.ШапкаДокумента.Вставить("ВычетЛиквидацииИностраннойОрганизации", 0);
	СтрокаДокумента.ШапкаДокумента.Вставить("ВычетДивидендовИностраннойОрганизации", 0);
	СтрокаДокумента.ШапкаДокумента.Вставить("ОпределениеПрибылиИностраннойОрганизации", 2); //1 - по правилам иностранной организации, 2 - по правилам РФ
	СтрокаДокумента.ШапкаДокумента.Вставить("ИндивидуальныйИнвестиционныйСчет", Ложь);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста 
Процедура ПроверитьШапкуВалютногоДохода2017(СтрокаДокумента)

	Если Не СтрокаДокумента.ШапкаДокумента.Свойство("КодВидаДохода") Тогда
		СтрокаДокумента.ШапкаДокумента.Вставить("КодВидаДохода", 2);
	КонецЕсли;
	Если Не СтрокаДокумента.ШапкаДокумента.Свойство("НомерИностраннойОрганизации") Тогда
		СтрокаДокумента.ШапкаДокумента.Вставить("НомерИностраннойОрганизации", "");
	КонецЕсли;
	Если Не СтрокаДокумента.ШапкаДокумента.Свойство("ВычетЛиквидацииИностраннойОрганизации") Тогда
		СтрокаДокумента.ШапкаДокумента.Вставить("ВычетЛиквидацииИностраннойОрганизации", 0);
	КонецЕсли;
	Если Не СтрокаДокумента.ШапкаДокумента.Свойство("ВычетДивидендовИностраннойОрганизации") Тогда
		СтрокаДокумента.ШапкаДокумента.Вставить("ВычетДивидендовИностраннойОрганизации", 0);
	КонецЕсли;
	Если Не СтрокаДокумента.ШапкаДокумента.Свойство("ОпределениеПрибылиИностраннойОрганизации") Тогда
		СтрокаДокумента.ШапкаДокумента.Вставить("ОпределениеПрибылиИностраннойОрганизации", 2);
	КонецЕсли;
	Если Не СтрокаДокумента.ШапкаДокумента.Свойство("ИндивидуальныйИнвестиционныйСчет") Тогда
		СтрокаДокумента.ШапкаДокумента.Вставить("ИндивидуальныйИнвестиционныйСчет", Ложь);
	КонецЕсли;
...
Процедура ПроверитьШапкуДохода2017(СтрокаДокумента)

	Если Не СтрокаДокумента.ШапкаДокумента.Свойство("ИндивидуальныйИнвестиционныйСчет") Тогда
		СтрокаДокумента.ШапкаДокумента.Вставить("ИндивидуальныйИнвестиционныйСчет", Ложь);
	КонецЕсли;
	Если Не СтрокаДокумента.ШапкаДокумента.Свойство("КадастроваяСтоимость") Тогда
		СтрокаДокумента.ШапкаДокумента.Вставить("КадастроваяСтоимость", 0);
	КонецЕсли;
	Если Не СтрокаДокумента.ШапкаДокумента.Свойство("ДоходБезКадастровойСтоимости") Тогда
		СтрокаДокумента.ШапкаДокумента.Вставить("ДоходБезКадастровойСтоимости", 0);
	КонецЕсли;
...
Процедура ОткрытьФормуДокументаДохода(ИДСтроки)
	...
	Если СтрокаДокумента.ТипДокумента = "2-НДФЛ" Тогда
		ИмяФормыРедактора = "ДоходПоСправке2НДФЛ2017кв1";
	ИначеЕсли СтрокаДокумента.ТипДокумента = "Прочий доход" Тогда
		ИмяФормыРедактора = "ДоходВРФПрочий2017кв1";
	ИначеЕсли СтрокаДокумента.ТипДокумента = "Доход в валюте" Тогда
		ИмяФормыРедактора = "ДоходВВалюте2017кв1";
	...
	ОткрытьФорму("Отчет.РегламентированныйОтчет3НДФЛ.Форма." + ИмяФормыРедактора, ПараметрыРедактораДокумента, ,,,, Оповещение);
...
Процедура ОткрытьФормуДоходаПредпринимателя(ИДСтроки)
	...
	ОткрытьФорму("Отчет.РегламентированныйОтчет3НДФЛ.Форма.ДоходПредпринимателя2015кв1", ПараметрыРедактораДокумента, ,,,, Оповещение);
...
Процедура ОткрытьФормуЖилья(ИДСтроки)
	...
	ОткрытьФорму("Отчет.РегламентированныйОтчет3НДФЛ.Форма.ОбъектИмущественногоВычета2015кв1", ПараметрыРедактораДокумента, ,,,, Оповещение);
...
Процедура ОткрытьФормуПенсионногоВзноса(ИДСтроки)
	...
	ОткрытьФорму("Отчет.РегламентированныйОтчет3НДФЛ.Форма.ВзносВПенсионныйФонд2015кв1", ПараметрыРедактораДокумента, ,,,, Оповещение);
...
Функция СтрокаРеквизитовПомощника()
	...
	Возврат "АдресВРФОКТМО,АдресВРФПредставление,АдресВРФСтруктура,АдресВРФТипАдреса,АдресЗаПределамиРФ,ВидДекларации,
		...
		|ИнвестиционныйВычетОтчетногоПериода,ИнвестиционныйВычетПрошлыхПериодов,ЯвляетсяОпекуном";
...
Функция СписокТаблицПомощника()
	...
	Результат.Добавить("ТаблицаУбытковПоЦеннымБумагамИИС");
...
Процедура ЗаполнитьТаблицыПомощникаДаннымиОтчета()
	...
	Если ЗначениеЗаполнено(СсылкаНаДокументОтчета) ИЛИ ЗначениеЗаполнено(Параметры.мСкопированаФорма) Тогда
		...
		Версия = Неопределено;
		Если Не ДанныеОтчета.Свойство("ВерсияФормы", Версия) 
			И ДанныеОтчета.Свойство("ДанныеПомощника") И ТипЗнч(ДанныеОтчета.ДанныеПомощника) = Тип("Структура") Тогда
			ДанныеОтчета.ДанныеПомощника.Свойство("ВерсияПомощника", Версия);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Версия) Или ВерсииФормыСовместимы(Версия, НомерВерсииФормыОтчета()) Тогда
		...
		Для Каждого ИмяТаблицы Из СписокТЧ Цикл
			...
			ИначеЕсли ИмяТаблицы = "ТаблицаДокументовФизЛица" Тогда
				...
				Для СчетчикСтрок = 0 По ВсегоСтрок Цикл
					...
					Если ЭтотОбъект[ИмяТаблицы][СчетчикСтрок].ТипДокумента = "Доход в валюте" 
						И ТипЗнч(ЭтотОбъект[ИмяТаблицы][СчетчикСтрок].ШапкаДокумента) = Тип("Структура") Тогда
						ПроверитьШапкуВалютногоДохода2017(ЭтотОбъект[ИмяТаблицы][СчетчикСтрок]);
					ИначеЕсли ДанныеПомощника[ИмяТаблицы].Колонки.Найти("ШапкаДокумента") <> Неопределено Тогда 
						ПроверитьШапкуДохода2017(ЭтотОбъект[ИмяТаблицы][СчетчикСтрок]);
					КонецЕсли;
...
Функция ПроверитьВерсиюДанныхПомощника(ДанныеПомощника, ВерсияФормы)
	...
	Результат = Ложь;
	ТекущаяВерсия = НомерВерсииФормыОтчета();
	
	Если ВерсияФормы = ТекущаяВерсия Тогда
		
		Возврат Истина;
		
	ИначеЕсли ВерсияФормы = "01/01/2017" Тогда
		
		ДанныеПомощника.Вставить("ЯвляетсяОпекуном", Ложь);
		
		Результат = Истина;
		
	ИначеЕсли ВерсияФормы = "10/01/2016" Тогда
	...
	ИначеЕсли ВерсияФормы = "01/01/2015" Или ВерсияФормы = "10/11/2015" Или ВерсияФормы = "10/01/2016" Тогда
		...
		// Проверяем таблицу убытков по ЦБ по ИИС
		Если ДанныеПомощника.Свойство("ТаблицаУбытковПоЦеннымБумагамИИС") Тогда
			
			ПредельныйГод = ГодОтчета - 1;
			НачальныйГод  = Макс(2015, ТекущийГод - 11);
			
			// Добавляем недостающие годы
			Для Счетчик = НачальныйГод По ПредельныйГод Цикл
				
				Если ДанныеПомощника.ТаблицаУбытковПоЦеннымБумагамИИС.Найти(Счетчик, "Год") = Неопределено Тогда
					СтрокаУбытка = ДанныеПомощника.ТаблицаУбытковПоЦеннымБумагамИИС.Добавить();
					СтрокаУбытка.Год = Счетчик;
				КонецЕсли;
			...
			// Удаляем лишние годы
			СтрокиКУдалению = Новый Массив;
			Для каждого СтрокаУбытка Из ДанныеПомощника.ТаблицаУбытковПоЦеннымБумагамИИС Цикл
				Если СтрокаУбытка.Год < МинимальныйГод Или СтрокаУбытка.Год > ПредельныйГод Тогда
					СтрокиКУдалению.Добавить(СтрокаУбытка);
				КонецЕсли;
			КонецЦикла;
			
			Для каждого СтрокаКУдалению Из СтрокиКУдалению Цикл
				ДанныеПомощника.ТаблицаУбытковПоЦеннымБумагамИИС.Удалить(СтрокаКУдалению);
			КонецЦикла;
...
Процедура ЗаписатьНаСервере(ОбновлятьДанныеОтчета = Ложь, ОтказатьсяОтПомощника = Ложь)
	...
	Если ЗначениеЗаполнено(СсылкаНаДокументОтчета) Тогда
		...
		ДокументОбъект = СсылкаНаДокументОтчета.ПолучитьОбъект();
		ДокументОбъект.ВыбраннаяФорма           = "ФормаОтчета2017кв1";
...
Процедура ПодготовитьСписокСохраненияОтчета(СписокСохранения, ПараметрыЗаполнения)
	...
	МакетСоставаПоказателей = ОбъектЭтогоОтчета().ПолучитьМакет("СоставПоказателей2017кв1");
	...
	СписокСохранения.ДанныеМногостраничныхРазделов.Вставить("ЛистЗ",             ПустаяТаблицаМногостраничногоРаздела());
	СписокСохранения.ДанныеМногостраничныхРазделов.Вставить("ЛистЗПродолжение",  ПустаяТаблицаМногостраничногоРаздела());
	СписокСохранения.ДанныеМногостраничныхРазделов.Вставить("ЛистЗПродолжение2", ПустаяТаблицаМногостраничногоРаздела());
	СписокСохранения.ДанныеМногостраничныхРазделов.Вставить("ЛистЗПродолжение3", ПустаяТаблицаМногостраничногоРаздела());
	СписокСохранения.ДанныеМногостраничныхРазделов.Вставить("ЛистЗПродолжение4", ПустаяТаблицаМногостраничногоРаздела());
	...
	СтраницаЛиста = ДобавитьВСписокСохраненияОтчетаСтраницуЛиста(СписокСохранения, ПараметрыЗаполнения, "ЛистЗ");
	СтраницаЛиста = ДобавитьВСписокСохраненияОтчетаСтраницуЛиста(СписокСохранения, ПараметрыЗаполнения, "ЛистЗПродолжение");
	СтраницаЛиста = ДобавитьВСписокСохраненияОтчетаСтраницуЛиста(СписокСохранения, ПараметрыЗаполнения, "ЛистЗПродолжение2");
	СтраницаЛиста = ДобавитьВСписокСохраненияОтчетаСтраницуЛиста(СписокСохранения, ПараметрыЗаполнения, "ЛистЗПродолжение3");
	СтраницаЛиста = ДобавитьВСписокСохраненияОтчетаСтраницуЛиста(СписокСохранения, ПараметрыЗаполнения, "ЛистЗПродолжение4");
...
Процедура ДобавитьСтруктуруПолейСтраницы(СтруктураПолейЛистов, ИмяСтраницы, Многостраничный = Ложь, Многострочный = Ложь)
	...
	ИмяМакета = "ФормаОтчета2017кв1_" + ИмяСтраницы;
...
Процедура ЗаполнитьОтчетДанными(СписокСохранения, ПараметрыЗаполнения)
	...
	СводДоходов.Колонки.Добавить("НеоблагаемыеДивидендыЗависимойКомпании",          ТипСумма);
	СводДоходов.Колонки.Добавить("НеоблагаемаяСтоимостьИмуществаЗависимойКомпании", ТипСумма);
	...
	ИтогиЛистов.Вставить("ЛистЕ1_п3_5_1", 0);
...
Процедура ЗаполнитьОтчетДокументамиФизЛица(СписокСохранения, ПараметрыЗаполнения)
	...
	Для каждого СтрокаДохода Из ТаблицаДокументовФизЛица Цикл
		...
		Если СтрокаДохода.ТипДокумента = "2-НДФЛ" Тогда
			...
			Для каждого СтрокаСправки Из СтрокаДохода.Строки2НДФЛ Цикл
				...
				Если СтрокаСправки.СуммаДохода > 0 Тогда
					...
					// Проверим наличие доходов гражданско-правового характера
					УчестьДоходНаЛистеЗ(СписокСохранения, ПараметрыЗаполнения, СтрокаСправки.КодДохода, СтрокаСправки.СуммаДохода, 
									СтрокаСправки.КодВычета, СтрокаСправки.СуммаВычета, СтрокаДохода.ШапкаДокумента.ИндивидуальныйИнвестиционныйСчет);
		...
		ИначеЕсли СтрокаДохода.ТипДокумента = "Прочий доход" Тогда
			...
			УчестьДоходНаЛистеЗ(СписокСохранения, ПараметрыЗаполнения, СтрокаДохода.ШапкаДокумента.КодДохода, СтрокаДохода.СуммаДохода, 
						СтрокаДохода.ШапкаДокумента.КодВычета, СтрокаДохода.СуммаВычета, СтрокаДохода.ШапкаДокумента.ИндивидуальныйИнвестиционныйСчет);
		...
		ИначеЕсли СтрокаДохода.ТипДокумента = "Доход в валюте" Тогда
			...
			СтрокаСводаДоходов.ОсобыйТипДохода = ОсобыйТипДохода(СтрокаДохода.ШапкаДокумента.КодДохода, СтрокаДохода.ШапкаДокумента.КодВидаДохода);
			Если СтрокаДохода.ШапкаДокумента.КодВидаДохода = 1 Тогда
				СтрокаСводаДоходов.НеоблагаемыеДивидендыЗависимойКомпании = СтрокаДохода.Шапка.ВычетДивидендовИностраннойОрганизации;
				СтрокаСводаДоходов.НеоблагаемаяСтоимостьИмуществаЗависимойКомпании = СтрокаДохода.Шапка.ВычетЛиквидацииИностраннойОрганизации;
			КонецЕсли;
			...
			// Проверим наличие имущественных вычетов
			УчестьДоходНаЛистеД2(СписокСохранения, ПараметрыЗаполнения, СтрокаДохода.ШапкаДокумента.КодДохода, СтрокаДохода.СуммаДохода, СтрокаДохода.ШапкаДокумента.КодВычета, СтрокаДохода.СуммаВычета
						, СтрокаДохода.ШапкаДокумента.ВычетЛиквидацииИностраннойОрганизации, СтрокаДохода.ШапкаДокумента.ВычетДивидендовИностраннойОрганизации);
			// Проверим наличие доходов гражданско-правового характера
			...
			// Проверим наличие доходов по ценным бумагам
			УчестьДоходНаЛистеЗ(СписокСохранения, ПараметрыЗаполнения, СтрокаДохода.ШапкаДокумента.КодДохода, 
						СтрокаДохода.СуммаДохода, СтрокаДохода.ШапкаДокумента.КодВычета, СтрокаДохода.СуммаВычета, 
						СтрокаДохода.ШапкаДокумента.ИндивидуальныйИнвестиционныйСчет);
	...
	Если СтрокиЛистаА.Количество() Тогда
		...
		Для каждого СтрокаДохода Из СтрокиЛистаА Цикл
			...
			ИначеЕсли СтрокаДохода.ТипДокумента = "Прочий доход" Тогда 
				ДанныеЗаполнения.Вставить("П000300001020", КодВидаДохода(СтрокаДохода.ШапкаДокумента.КодДохода, 
								СтрокаДохода.ШапкаДокумента.КадастроваяСтоимость, СтрокаДохода.ШапкаДокумента.ДоходБезКадастровойСтоимости)); // Код вида дохода
			Иначе
	...
	Если СтавкиВалютныхДоходов.Количество() > 0 Тогда
		...
		Для каждого ДоходыПоСтавке Из СтавкиВалютныхДоходов Цикл
			...
			Для каждого СтрокаДохода Из ДоходыПоСтавке.Значение Цикл
				...
				ДанныеЗаполнения.Вставить("П000400001031", СтрокаДохода.ШапкаДокумента.КодВидаДохода); // код вида дохода
				...
				Если СтрокаДохода.ШапкаДокумента.КодВидаДохода = 1 Тогда
					ДанныеЗаполнения.Вставить("П000400001032", СтрокаДохода.ШапкаДокумента.НомерИностраннойОрганизации); // Номер контролируемой иностранной организации
					ДанныеЗаполнения.Вставить("П000400001071", СтрокаДохода.ШапкаДокумента.ВычетЛиквидацииИностраннойОрганизации); // Необлгаемая стоимость имущества ликвидируемой зависимой иностранной компании
					ДанныеЗаполнения.Вставить("П000400001072", СтрокаДохода.ШапкаДокумента.ВычетДивидендовИностраннойОрганизации); // Необлагаемая сумма дивидендов от зависимой иностранной компании
					ДанныеЗаполнения.Вставить("П000400001073", СтрокаДохода.ШапкаДокумента.ОпределениеПрибылиИностраннойОрганизации); // способ определения прибыли зависимой иностранной компании
				КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ОсобыйТипДохода(КодДохода, КодВидаДохода = Неопределено)

	Если СтрНайти("4910,1000,1010,", КодДохода + ",") > 0 Тогда
		Возврат "Дивиденды";
	ИначеЕсли КодВидаДохода = 1 
		Или СтрНайти("9811,9812,", КодДохода + ",") > 0 Тогда
		Возврат "ПрибыльИКО";
	Иначе
		Возврат "";
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция КодВидаДохода(КодДохода, КадастроваяСтоимость = Неопределено, ДоходБезКадастровойСтоимости= Неопределено)

	Если СтрНайти("1510,1511,1550,", КодДохода + ",") > 0 Тогда
		Если ЗначениеЗаполнено(КадастроваяСтоимость) И ЗначениеЗаполнено(ДоходБезКадастровойСтоимости)
			И ДоходБезКадастровойСтоимости < КадастроваяСтоимость * 0.7 Тогда
			Возврат "09"; //Доход от продажи недвижимого имущества и долей в нем, определенный исходя из кадастровой стоимости этого объекта, умноженной на понижающий коэффициент 0,7
		Иначе
	...
	ИначеЕсли Лев(КодДохода, 3) = "153" Тогда
...
Функция КодВидаДоходаПоСтрокамСправки(Строки2НДФЛ)
	...
	Для каждого СтрокаСправки Из Строки2НДФЛ Цикл
		...
		ИначеЕсли (Лев(КодДохода, 3) = "200" ИЛИ Лев(КодДохода, 3) = "201" ИЛИ Лев(КодДохода, 3) = "230") Тогда
		...
		Иначе
			...
			КодВидаДохода = КодПрочие;  // Иные доходы
			Прервать;
...
//  Процедура вызываетя из ЗаполнитьОтчетДокументамиФизЛица()
&НаСервере
Процедура УчестьДоходНаЛистеД2(СписокСохранения, ПараметрыЗаполнения, КодДохода, СуммаДохода, КодВычета, СуммаВычета, ВычетЛиквидацииИностраннойОрганизации = 0, ВычетДивидендовИностраннойОрганизации = 0)
	...
	ИначеЕсли КодДохода = "9811" И КодВычета = "911"  Тогда //прочее имущество по факту
		
		ДанныеЛиста.П000800001170 = ДанныеЛиста.П000800001170 + СуммаДохода;
		ДанныеЛиста.П000800001180 = Мин(ДанныеЛиста.П000800001170, ВычетЛиквидацииИностраннойОрганизации);
		
	ИначеЕсли КодДохода = "9812" И КодВычета = "912"  Тогда //прочее имущество по факту
		
		ДанныеЛиста.П000800001190 = ДанныеЛиста.П000800001190 + СуммаДохода;
		ДанныеЛиста.П000800001200 = Мин(ДанныеЛиста.П000800001190, ВычетДивидендовИностраннойОрганизации);
	...
	// Пересчитываем итоги листа:
	ДанныеЛиста.П000800001210 = ДанныеЛиста.П000800001020 + ДанныеЛиста.П000800001040 + ДанныеЛиста.П000800001060
					...
					+ ДанныеЛиста.П000800001180 + ДанныеЛиста.П000800001200; 
	
	ПараметрыЗаполнения.ЗаполненныеРазделы.Вставить("ЛистД2", ДанныеЛиста.П000800001210 <> 0);

	ПараметрыЗаполнения.ИтогиЛистов.ЛистД2_п4 = ДанныеЛиста.П000800001210;
...
Процедура ЗаполнитьЛистЕ1(СписокСохранения, ПараметрыЗаполнения)
	...
	Для Каждого СтрокаДохода Из ТаблицаДоходов Цикл
		...
		Если ВсегоДоход < 350000 Тогда
			МесяцевДо350000 = МесяцевДо350000 + 1;
		КонецЕсли; 
		СоответствиеМесяцев.Вставить(СтрокаДохода.Месяц, СтрокаДохода);
	КонецЦикла; 
	
	// Указываем количесво месяцев
	ДанныеЛиста.П000900001030 = МесяцевДо350000;
	...
	Если НужныВычетыНаРаботника Тогда
		...
		ВычетИнвалида              = ?(ЯвляетсяОпекуном, 6000, 12000);
		...
		Для Каждого СтрокаВычета Из ТаблицаСтандартныхВычетов Цикл
			...
			Если МесяцевДо350000 > 0 И СтрокаВычета.ВычетНаДетей > 0 
				И СоответствиеМесяцев[СтрокаВычета.НомерМесяца].ДоходНарастающим < 350000 Тогда
				...
				Если СтрокаВычета.ВычетНаДетей = 1 Тогда // полная семья
					...
					ИнвалидамВПолнойСемье = ИнвалидамВПолнойСемье + СтрокаВычета.КоличествоДетейИнвалидов * ВычетИнвалида;
				Иначе // единственный родитель
					ДетямОдинокогоРодителя = ДетямВПолнойСемье
							+ (СтрокаВычета.КоличествоДетейДоТретьего * 1400 
							+ СтрокаВычета.КоличествоДетейСТретьего * 3000) * 2;
					ИнвалидамОдинокогоРодителя = ИнвалидамВПолнойСемье + СтрокаВычета.КоличествоДетейИнвалидов * ВычетИнвалида * 2;
				КонецЕсли; 
		...
		ДанныеЛиста.П000900001050 = ДетямОдинокогоРодителя;
		ДанныеЛиста.П000900001060 = ИнвалидамВПолнойСемье;
	...
	Если НужныДругиеСоцВычеты Тогда
		...
		ДанныеЛиста.П000900001171 = ДополнительныеВзносыПФВычетУНалоговогоАгента;
		ДанныеЛиста.П000900001180 = ДанныеЛиста.П000900001120 + ДанныеЛиста.П000900001170 - ДанныеЛиста.П000900001171;
	...
	ПараметрыЗаполнения.ИтогиЛистов.ЛистЕ1_п3_5_1 = ДанныеЛиста.П000900001171;
...
Функция ПолучитьСтраницуЛистаЗ(СписокСохранения, ИмяЛиста, ВидОперации, ПараметрыЗаполнения) 

	ЛистЗ = Неопределено;
	Для Каждого СтрокаЛиста Из СписокСохранения.ДанныеМногостраничныхРазделов[ИмяЛиста] Цикл
		Если Строка(СтрокаЛиста.Данные.П001200001100) = Строка(ВидОперации) ИЛИ ПустаяСтрока(СтрокаЛиста.Данные.П001200001100) Тогда
			ЛистЗ = СтрокаЛиста;
			Прервать;
		КонецЕсли; 
	КонецЦикла; 

	Если ЛистЗ = Неопределено Тогда
		ЛистЗ = ДобавитьВСписокСохраненияОтчетаСтраницуЛиста(СписокСохранения, ПараметрыЗаполнения, ИмяЛиста);
	КонецЕсли; 
	
	ЛистЗ.Данные.П001200001100 = ВидОперации;
...
&НаСервере
Процедура УчестьДоходНаЛистеЗ(СписокСохранения, ПараметрыЗаполнения, КодДохода, СуммаДохода, КодВычета, СуммаВычета, ИИС)
	...
	ЛистЗ             = ПолучитьСтраницуЛистаЗ(СписокСохранения, "ЛистЗ", ВидОперации, ПараметрыЗаполнения).Данные;
	ЛистЗПродолжение  = ПолучитьСтраницуЛистаЗ(СписокСохранения, "ЛистЗПродолжение", ВидОперации, ПараметрыЗаполнения).Данные;
	ЛистЗПродолжение2 = ПолучитьСтраницуЛистаЗ(СписокСохранения, "ЛистЗПродолжение2", ВидОперации, ПараметрыЗаполнения).Данные;
	ЛистЗПродолжение3 = ПолучитьСтраницуЛистаЗ(СписокСохранения, "ЛистЗПродолжение3", ВидОперации, ПараметрыЗаполнения).Данные;
	ЛистЗПродолжение4 = ПолучитьСтраницуЛистаЗ(СписокСохранения, "ЛистЗПродолжение4", ВидОперации, ПараметрыЗаполнения).Данные;
...
Процедура ЗаполнитьИтогиЛистаЗ(СписокСохранения, ПараметрыЗаполнения)
	...
	ПараметрыЗаполнения.ИтогиЛистов.ЛистЗ_п11_3 = 0;
	Для каждого СтрокаЛиста Из СписокСохранения.ДанныеМногостраничныхРазделов.ЛистЗ Цикл
		
		ВидОперации = Строка(СтрокаЛиста.Данные.П001200001100);
		ЗаполнитьИтогиЛистаЗПоВидуОперации(СписокСохранения, ПараметрыЗаполнения, ВидОперации);
...
&НаСервере
Процедура ЗаполнитьИтогиЛистаЗПоВидуОперации(СписокСохранения, ПараметрыЗаполнения, ВидОперации)
	...
	// Общие суммы убытков
	ТаблицаУбытов = ?(ВидОперации = 1, ТаблицаУбытковПоЦеннымБумагамИИС, ТаблицаУбытковПоЦеннымБумагам);
	УбытокПоЦеннымБумагам         = ТаблицаУбытов.Итог("УбытокПоЦеннымБумагам");
	УбытокПоФинансовымИнстументам = ТаблицаУбытов.Итог("УбытокПоФинансовымИнстументам");
	 
	ЛистЗПродолжение4.П001600001501 = УбытокПоЦеннымБумагам;
	ЛистЗПродолжение4.П001600001512 = УбытокПоФинансовымИнстументам;
	// В том числе по годам
	КоличествоЛет = ?(ВидОперации = 1 , 1, 10);
	Для Счетчик = 0 По КоличествоЛет - 1 Цикл
		СуффиксЯчейки1 = Формат(Счетчик + 2, "ЧЦ=2; ЧДЦ=; ЧВН=");
		СуффиксЯчейки2 = Строка(Счетчик + 13);
		ЛистЗПродолжение4["П0016000015" + СуффиксЯчейки1 + "_год"] = ТаблицаУбытов[Счетчик].Год;
		ЛистЗПродолжение4["П0016000015" + СуффиксЯчейки1] = ТаблицаУбытов[Счетчик].УбытокПоЦеннымБумагам;
		ЛистЗПродолжение4["П0016000015" + СуффиксЯчейки2] = ТаблицаУбытов[Счетчик].УбытокПоФинансовымИнстументам;
	КонецЦикла; 
	...
	Если УбытокПоЦеннымБумагам <> 0 Тогда
	...
	Если УбытокПоФинансовымИнстументам <> 0 Тогда
	...
	ЛистЗПродолжение4.П001600001531 = ЛистЗ.П001200001101 // п. 1.1
					+ ЛистЗПродолжение.П001300001201 // п. 2.1
					+ ЛистЗПродолжение.П001300001205 // п. 3.1
					+ ЛистЗПродолжение2.П001400001301 // п 4.1
					+ ЛистЗПродолжение2.П001400001312 // п 5.1
					+ ЛистЗПродолжение2.П001400001321 // п 6.1
					+ ЛистЗПродолжение3.П001500001401 // п 7.1
					+ ЛистЗПродолжение3.П001500001407; // п 8.1
	...
	ПараметрыЗаполнения.ИтогиЛистов.ЛистЗ_п11_3 = ПараметрыЗаполнения.ИтогиЛистов.ЛистЗ_п11_3 + ЛистЗПродолжение4.П001600001533;
...
Процедура ЗаполнитьРаздел2(СписокСохранения, ПараметрыЗаполнения)
	...
	// Сворачиваем таблицу доходов:
	СводДоходов = ПараметрыЗаполнения.СводДоходов;
	СводДоходов.Свернуть("СтавкаНалога,ОКТМО,КБК,ОсобыйТипДохода", "СуммаДохода,СуммаВычета,НалогУдержан,НалогУплачен,
						|НалогПринимаемыйКУчету,НеоблагаемыеДивидендыЗависимойКомпании,НеоблагаемаяСтоимостьИмуществаЗависимойКомпании");
	...
	СписокКодовВидовДохода = Новый Массив;
	СписокКодовВидовДохода.Добавить("Дивиденды");
	СписокКодовВидовДохода.Добавить("ПрибыльИКО");
	СписокКодовВидовДохода.Добавить("");

	ИтогиЛистов = ПараметрыЗаполнения.ИтогиЛистов;
	
	Для Каждого СтавкаНалога Из СписокСтавок Цикл
		
		НеоблагаемыеДивидендыЗависимойКомпании          = 0;
		НеоблагаемаяСтоимостьИмуществаЗависимойКомпании = 0;
		
		Для каждого ОсобыйТипДохода Из СписокКодовВидовДохода Цикл
			
			СтрокиДохода = СводДоходов.НайтиСтроки(Новый Структура("СтавкаНалога,ОсобыйТипДохода", СтавкаНалога, ОсобыйТипДохода));
			...
			// Вид дохода 002
			Если ОсобыйТипДохода= "Дивиденды" Тогда
				ДанныеЛиста.П000200001002 = "1";
			ИначеЕсли ОсобыйТипДохода= "ПрибыльИКО" Тогда
				ДанныеЛиста.П000200001002 = "2";
			Иначе
				ДанныеЛиста.П000200001002 = "3";
			КонецЕсли;
			
			// 010 - Общая сумма доходов и 080 - Общая сумма налога, удержанная у источника выплаты
			Для Каждого СтрокаДохода Из СтрокиДохода Цикл
				
				Если ОсобыйТипДохода = "" Тогда
					
					ДанныеЛиста.П000200001010 = ДанныеЛиста.П000200001010 + СтрокаДохода.СуммаДохода; //1. Общая сумма дохода
					
				ИначеЕсли ОсобыйТипДохода = "Дивиденды" Тогда
					
					// Общая сумма дивидендов отражается в строке 010 отдельно от иных доходов, полученных в налоговом периоде, 
					//и определяется исходя из сумм данных видов доходов, облагаемых по соответствующей налоговой ставке, 
					//указанных в строках 080 Листа А и в строках 070 Листа Б.
					ДанныеЛиста.П000200001010 = ДанныеЛиста.П000200001010 + СтрокаДохода.СуммаДохода - СтрокаДохода.СуммаВычета;
					
				ИначеЕсли ОсобыйТипДохода = "ПрибыльИКО" Тогда
					
					// Общая сумма доходов в виде сумм прибыли контролируемых иностранных компаний (строка 051) 
					//определяется исходя из сумм данного вида доходов, облагаемых по соответствующей налоговой 
					//ставке, указанных в строках 070 Листа Б.
					ДанныеЛиста.П000200001051 = ДанныеЛиста.П000200001051 + СтрокаДохода.СуммаДохода; 
					НеоблагаемыеДивидендыЗависимойКомпании          = НеоблагаемыеДивидендыЗависимойКомпании + СтрокаДохода.НеоблагаемыеДивидендыЗависимойКомпании;
					НеоблагаемаяСтоимостьИмуществаЗависимойКомпании = НеоблагаемаяСтоимостьИмуществаЗависимойКомпании + СтрокаДохода.НеоблагаемаяСтоимостьИмуществаЗависимойКомпании;
					
				КонецЕсли;
				
				//	080 - Общая сумма налога, удержанная у источника выплаты
				ДанныеЛиста.П000200001080 = ДанныеЛиста.П000200001080 + СтрокаДохода.НалогУдержан; 
				
			КонецЦикла; 
			
			// Заполнение строки 020 зависит от вида дохода с учетом ставки и статуса налогоплательщика
			Если ОсобыйТипДохода = "" Тогда
				
				Если СтавкаНалога = 13 И НалогоплательщикСтатус = 1 Тогда
					
					// К сумме дохода в строке 010 добавляются суммы ранее предоставленного инвестиционного налогового вычета, 
					//	указанной в подпункте 3.2 Листа Е2, подлежащей восстановлению
					ДанныеЛиста.П000200001010 = ДанныеЛиста.П000200001010 + ИнвестиционныйВычетПрошлыхПериодов;
					
					// 020 - необлашаемые суммы доходов переносятся с итогов листа Г
					ДанныеЛиста.П000200001020 = ИтогиЛистов.ЛистГ_п10; 
					
					// Сумма налоговых вычетов
					//(пп. 3.2 Листа В + пп. 2.5 Листа Д1 + пп. 2.6 Листа Д1 
					//+ пп. 2.8 Листа Д1 + пп. 2.9 Листа Д1 + п. 4 Листа Д2
					//+ п. 3.5.1 Листа Е1 + п. 4 Листа Е1 + п. 3.1 Листа Е2 
					//+ пп. 8.1. Листа Ж )
					ДанныеЛиста.П000200001040 = ИтогиЛистов.ЛистВ_п3_2 + ИтогиЛистов.ЛистД1_п2_5 + ИтогиЛистов.ЛистД1_п2_6
												 ...
												 + ИтогиЛистов.ЛистЕ1_п3_5_1 + ИтогиЛистов.ЛистЕ1_п4  + ИтогиЛистов.ЛистЕ2_п3_1 
												 + ИтогиЛистов.ЛистЖ_п8_1;
												 
					// 5. Сумма расходов, принимаемая в уменьшение полученных доходов (по ЦБ)
					ДанныеЛиста.П000200001050 = ИтогиЛистов.ЛистЗ_п11_3 + ИтогиЛистов.ЛистИ_п7_3; 
			...
			ИначеЕсли ОсобыйТипДохода = "ПрибыльИКО" Тогда
				...
				// 020 - необлашаемые суммы доходов переносятся переносится из строк 071 и 072 Листа Б
				ДанныеЛиста.П000200001020 = НеоблагаемыеДивидендыЗависимойКомпании + НеоблагаемаяСтоимостьИмуществаЗависимойКомпании; 
				
			КонецЕсли;
			
			// 030 - 3. Общая сумма дохода, подлежащая налогообложению 
			ДанныеЛиста.П000200001030 = Макс(0, ДанныеЛиста.П000200001010 - ДанныеЛиста.П000200001020);
			...
			// 060 - 6. Налоговая база для исчисления налога (п. 3 + п. 5.1 - п. 4 - п. 5)
			ДанныеЛиста.П000200001060 = Макс(0, ДанныеЛиста.П000200001030 + ДанныеЛиста.П000200001051 
							- ДанныеЛиста.П000200001040 - ДанныеЛиста.П000200001050); 
			
			// 2. Расчет суммы налога
			// 070 - 7. Общая сумма налога, исчисленная к уплате, определяется умножением ставки на сумму дохода
			ДанныеЛиста.П000200001070 = Окр(ДанныеЛиста.П000200001060 * СтавкаНалога / 100, 0); 
			
			// 080 заполнена по строкам доходов
			
			// 091 - 10. Сумма торгового сбора, уплаченная в налоговом периоде, подлежащая зачету
			Если ОсобыйТипДохода = "" И 
					(СтавкаНалога = 13 И НалогоплательщикСтатус = 1 Или СтавкаНалога = 30 И НалогоплательщикСтатус = 2) Тогда
					
				// значение показателя строки 091 не может превышать значение показателя строки 070
				ДанныеЛиста.П000200001091 = Мин(ДанныеЛиста.П000200001070, ПредпринимательТорговыйСбор); 
				
				// 100 - 10. Сумма налога, уплаченная налогоплательщиком, подлежащая зачёту (авансовые платежи предпринимателя)
				ДанныеЛиста.П000200001100 = ИтогиЛистов.ЛистВ_п3_4; 
				
			Иначе
				ДанныеЛиста.П000200001091 = 0; 
				ДанныеЛиста.П000200001100 = 0;
			КонецЕсли;
			
			// 110 - 12. Общая сумма фиксированных авансовых платежей, уплаченная налогоплательщиком, подлежащая зачету в соответствии 
			//с пунктом 5 статьи 227.1 Налогового Кодекса Российской Федерации
			ДанныеЛиста.П000200001110 = ?(ОсобыйТипДохода = "" И СтавкаНалога = 13, ДоходыФЛФиксированныеПлатежиИностранца, 0); 
			
			// 120 - 13. Сумма налога в иностранных государствах, подлежащая зачету в РФ
			...
			ДанныеЛиста.П000200001121 = Макс(0, ДанныеЛиста.П000200001070 - ДанныеЛиста.П000200001080 - ДанныеЛиста.П000200001090
						 - ДанныеЛиста.П000200001091 - ДанныеЛиста.П000200001100 - ДанныеЛиста.П000200001110 - ДанныеЛиста.П000200001120);
			
			// 122 - 14.1. Сумма налога, уплаченная в связи с применением патентной системы налогообложения, подлежащая зачету
			Если ОсобыйТипДохода = "" И 
					(СтавкаНалога = 13 И НалогоплательщикСтатус = 1 Или СтавкаНалога = 30 И НалогоплательщикСтатус = 2) Тогда
			...
			// 130 - 15 Сумма к уплате
			ДанныеЛиста.П000200001130 = Макс(0, ДанныеЛиста.П000200001121 - ДанныеЛиста.П000200001122);
			
			// 140 - 16. сумма к возврату
			ДанныеЛиста.П000200001140 = Макс(0, ДанныеЛиста.П000200001080 + ДанныеЛиста.П000200001090 + ДанныеЛиста.П000200001091
			...
			ЕстьВычетыПоСтавке   = ДанныеЛиста.П000200001010 > ДанныеЛиста.П000200001060 Или ОсобыйТипДохода <> "";
...
Процедура ВывестиБланкДекларации(ТипБланка, БезПоказа = Ложь)
	...
	ФормаОтчета = ПолучитьФорму("Отчет.РегламентированныйОтчет3НДФЛ.Форма." + ПараметрыФормы.мВыбраннаяФорма, ПараметрыФормы);
...
Процедура ПродолжитьБезПомощникаЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Если НЕ Модифицированность Тогда
			...
			ПараметрыФормы = ПараметрыФормыРеглОтчета();
			ПараметрыФормы.Вставить("БезОткрытияФормы", Ложь);
			ОткрытьФорму("Отчет.РегламентированныйОтчет3НДФЛ.Форма.ФормаОтчета2017кв1", ПараметрыФормы);
...
Функция ПараметрыФормыРеглОтчета()
	...
	ПараметрыДляФормыОтчета.Вставить("мВыбраннаяФорма",          "ФормаОтчета2017кв1");
...
Функция ВыполнитьЗагрузкуДанныхИзДругогоОтчета(СсылкаНаОтчет, ТекстСообщений)
	...
	Попытка
		...
		Если ВерсияОтчета = "01/01/2014" Тогда
			...
			КонецЕсли; 
		ИначеЕсли ВерсияОтчета = "01/01/2015" Или  ВерсияОтчета = "10/11/2015" Или  ВерсияОтчета = "10/01/2015"
				Или ВерсияОтчета = "01/01/2016" Или ВерсияОтчета = "10/01/2016" Тогда
			Если ЕстьПомощник Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция ВерсииФормыСовместимы(ВерсияФормы, ТекущаяВерсия)

	Если ВерсияФормы = ТекущаяВерсия Тогда
		Возврат Истина;
		
	ИначеЕсли ВерсияФормы = "01/01/2017" И ТекущаяВерсия = "02/01/2017" Тогда
