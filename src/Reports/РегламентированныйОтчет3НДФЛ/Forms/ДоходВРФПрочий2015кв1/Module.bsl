...
Процедура КодДоходаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "ДоходВРФПрочий2015кв1", "ВыборКодаДоходаИВычета"), ПараметрыФормы, Элемент, Истина, , , Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Функция СтрокаСвойствШапкиДокумента()
	...
	Возврат "ИсточникНаименование,ИсточникИНН,
				...
				|КодВычета,НаименованиеВычета,КодыВычетов,СуммаВычета,Месяц";
