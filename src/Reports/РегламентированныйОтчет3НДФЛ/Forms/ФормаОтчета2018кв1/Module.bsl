...
&НаКлиенте
Процедура АктивизироватьРаздел(ИмяРаздела, ИндексЛиста = 0) Экспорт
	
	Для Каждого Раздел Из РазделыОтчета.ПолучитьЭлементы() Цикл
		Если Раздел.КолонкаРазделыОтчетаСокрНаим = ИмяРаздела
			И Раздел.КолонкаНомерСтраницыРазделаОтчета = ИндексЛиста Тогда
			Элементы.РазделыОтчета.ТекущаяСтрока = Раздел.ПолучитьИдентификатор();
			РазделОтчетаПриИзменении(Раздел, Элементы.РазделыОтчета);
		КонецЕсли;
	КонецЦикла;
