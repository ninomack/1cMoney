////////////////////////////////////////////////////////////////////////////////
// Модуль формы <ПомощникЗаполнения2015кв1>
...
Процедура КодИФНСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "ПомощникЗаполнения2015кв1", "ВыборНалоговойИнспекции"), ПараметрыФормы, Элемент);
...
Процедура ТаблицаИмущественныхВычетовПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Иначе
		...
		НоваяСтрока.ЖильеГодНачалаВычета    = 2014;
...
Процедура ОткрытьФормуДокументаДохода(ИДСтроки)
	...
	ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "ПомощникЗаполнения2015кв1", ИмяФормыРедактора), ПараметрыРедактораДокумента, ,,,, Оповещение);
...
Процедура ОткрытьФормуДоходаПредпринимателя(ИДСтроки)
	...
	ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "ПомощникЗаполнения2015кв1", "ДоходПредпринимателя2015кв1"), ПараметрыРедактораДокумента, ,,,, Оповещение);
...
Процедура ОткрытьФормуЖилья(ИДСтроки)
	...
	ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "ПомощникЗаполнения2015кв1", "ОбъектИмущественногоВычета2015кв1"), ПараметрыРедактораДокумента, ,,,, Оповещение);
...
Процедура ОткрытьФормуПенсионногоВзноса(ИДСтроки)
	...
	СтрокаСвойств = "ИДЭлемента,Представление,СуммаВзноса,ИНН,КПП,НаименованиеОрганизации,НомерДоговора,ДатаДоговора,ВычетУНалоговогоАгента,ПринимаетсяКВычету,ВидДоговора";
	...
	ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "ПомощникЗаполнения2015кв1", "ВзносВПенсионныйФонд2015кв1"), ПараметрыРедактораДокумента, ,,,, Оповещение);
...
Процедура ОткрытьФормуПенсионногоВзносаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		СтрокаСвойств = "Представление,СуммаВзноса,ИНН,КПП,НаименованиеОрганизации,НомерДоговора,ДатаДоговора,ВычетУНалоговогоАгента,ПринимаетсяКВычету";
...
Функция СтрокаРеквизитовПомощника()
	...
	Возврат "АдресВРФОКТМО,АдресВРФПредставление,АдресВРФСтруктура,АдресВРФТипАдреса,АдресЗаПределамиРФ,ВидДекларации,
		...
		|ПредпринимательНеоблагаемыйДоходКФХ,ПредпринимательУплаченоФактическихПлатежей";
...
Процедура ЗаписатьНаСервере(ОбновлятьДанныеОтчета = Ложь, ОтказатьсяОтПомощника = Ложь)
	...
	Если ЗначениеЗаполнено(СсылкаНаДокументОтчета) Тогда
		...
		ДокументОбъект = СсылкаНаДокументОтчета.ПолучитьОбъект();
		ДокументОбъект.ВыбраннаяФорма           = "ФормаОтчета2015кв1";
...
Функция ПолучитьСтруктуруДанныхПомощникаЗаполнения()
	...
	Результат.Вставить("ВерсияСтруктурыДанныхПомощника", "20150110");
...
Процедура ПодготовитьСписокСохраненияОтчета(СписокСохранения, ПараметрыЗаполнения)
	...
	СписокСохранения.Вставить("ВерсияФормы",                               "10/11/2015");
	...
	МакетСоставаПоказателей = ОбъектЭтогоОтчета().ПолучитьМакет("СоставПоказателей2015кв1");
...
Процедура ДобавитьСтруктуруПолейСтраницы(СтруктураПолейЛистов, ИмяСтраницы, Многостраничный = Ложь, Многострочный = Ложь)
	...
	ИмяМакета = "ФормаОтчета2015кв1_" + ИмяСтраницы;
...
Процедура ЗаполнитьОтчетДанными(СписокСохранения, ПараметрыЗаполнения)
	...
	ИтогиЛистов.Вставить("ЛистЕ2_п3", 0);
	...
	ИтогиЛистов.Вставить("ЛистЖ_п6_1", 0);
...
Процедура ЗаполнитьОтчетДокументамиФизЛица(СписокСохранения, ПараметрыЗаполнения)
	...
	Для каждого СтрокаДохода Из ТаблицаДокументовФизЛица Цикл
		...
		Если СтрокаДохода.ТипДокумента = "2-НДФЛ" Тогда
			...
			Для каждого СтрокаСправки Из СтрокаДохода.Строки2НДФЛ Цикл
				...
				Если СтрокаСправки.СуммаДохода > 0 Тогда
					...
					Если СтрокаДохода.ИспользоватьДляСтандартныхВычетов И СтрокаДохода.СтавкаНалога = 13 Тогда
				...
				Если СтрокаСправки.КодВычета <> "320" И СтрокаСправки.КодВычета <> "320" Тогда
		...
		ИначеЕсли СтрокаДохода.ТипДокумента = "Прочий доход" Тогда
			...
			// Добавляем строку для расчета стандартных вычетов
			Если СтрокаДохода.ИспользоватьДляСтандартныхВычетов И СтрокаДохода.СтавкаНалога = 13 И СтрокаДохода.ШапкаДокумента.Свойство("Месяц") Тогда
				СтрокаСтандартныхВычетов = ПараметрыЗаполнения.ИсточникСтандартныхВычетов.Добавить();
	...
	Если СтавкиВалютныхДоходов.Количество() > 0 Тогда
		...
		Для каждого ДоходыПоСтавке Из СтавкиВалютныхДоходов Цикл
			...
			ДанныеИтогов.Вставить("П000400001140", 0); // Всего получено в пересчете в рубли
			ДанныеИтогов.Вставить("П000400001150", 0); // Всего уплачено налога за границей в пересчете в рубли
			ДанныеИтогов.Вставить("П000400001160", 0); // Всего налог, исчисленный в РФ
			ДанныеИтогов.Вставить("П000400001170", 0); // Всего налог, принимаемый к зачету в РФ
			...
			Для каждого СтрокаДохода Из ДоходыПоСтавке.Значение Цикл
				...
				ДанныеИтогов.П000400001140 = ДанныеИтогов.П000400001140 + СтрокаДохода.СуммаДохода;
				ДанныеИтогов.П000400001150 = ДанныеИтогов.П000400001150 + СтрокаДохода.ШапкаДокумента.СуммаНалогаУплаченоЗаГраницей;
				ДанныеИтогов.П000400001160 = ДанныеИтогов.П000400001160 + СтрокаДохода.СуммаНалогаНачислено;
				ДанныеИтогов.П000400001170 = ДанныеИтогов.П000400001170 + СтрокаДохода.ШапкаДокумента.СуммаНалогаПринимаетсяКЗачету;
				
			КонецЦикла;
			
			// На последем листе по каждой ставе выводим итог
			ДобавитьНаСтраницуЛистаПоляИтогов(ЛистБ, ПараметрыЗаполнения, "ЛистБ", ДанныеИтогов);
			
			// Запоминаем итог листа Б для конкретной ставки
			ПараметрыЗаполнения.ИтогиЛистов.ЛистБ_п2_4.Вставить("Ставка" + ДоходыПоСтавке.Ключ, ЛистБ.Данные.П000400001170);
...
Функция КодВидаДохода(КодДохода)
	...
	ИначеЕсли СтрНайти("1520,", КодДохода + ",") > 0 Тогда
...
Процедура УчестьДоходНаЛистеГ(СписокСохранения, ПараметрыЗаполнения, КодДохода, СуммаДохода, КодВычета, СуммаВычета, НомерМесяца = 0)
	...
	Если КодВычета = "320" Тогда //Сумма уплаченных работодателем доп. страховых взносов в ПФ
				// (является вычетом)
		
		//ДанныеЛиста.П000600001150 = ДанныеЛиста.П000600001150 + СуммаВычета;
		//ДанныеЛиста.П000600001160 = Мин(ДанныеЛиста.П000600001150, 12000);
		//ВыводитьЛистГ = Истина;
...
Процедура УчестьДоходНаЛистеД2(СписокСохранения, ПараметрыЗаполнения, КодДохода, СуммаДохода, КодВычета, СуммаВычета)
	...
	Если КодДохода = "1510" И КодВычета = "301" Тогда	// недвижимость, кроме долей = по норме
		...
		ДанныеЛиста.П000800001020 = Мин(ДанныеЛиста.П000800001020 + СуммаВычета, ДанныеЛиста.П000800001010, 1000000);
	...
	ИначеЕсли КодДохода = "1511" И КодВычета = "302"  Тогда //доли в недвижимости =по норме
		...
		ДанныеЛиста.П000800001040 = Мин(ДанныеЛиста.П000800001040 + СуммаВычета, ДанныеЛиста.П000800001030, 1000000);
		
	ИначеЕсли (КодДохода = "1510" ИЛИ КодДохода = "1511") И КодВычета = "305"  Тогда //недвижимость, включая доли, по факту
		
		ДанныеЛиста.П000800001070 = ДанныеЛиста.П000800001070 + СуммаДохода;
		ДанныеЛиста.П000800001080 = Мин(ДанныеЛиста.П000800001070, ДанныеЛиста.П000800001080 + СуммаВычета);
	...
	ИначеЕсли КодДохода = "1520" И КодВычета = "303"  Тогда //прочее имущество по норме
		...
		ДанныеЛиста.П000800001120 = Мин(ДанныеЛиста.П000800001120 + СуммаВычета, ДанныеЛиста.П000800001110, 250000);
	...
	ИначеЕсли КодВычета = "908"  Тогда //Вычет из выкупной стоимости изъятого для гос. нужд имущества
		...
		ДанныеЛиста.П000800001180 = ДанныеЛиста.П000800001180 + СуммаВычета;
		
	КонецЕсли; 

	// Пересчитываем итоги листа:
	ДанныеЛиста.П000800001050 = ДанныеЛиста.П000800001010 + ДанныеЛиста.П000800001030; 
	ДанныеЛиста.П000800001060 = Мин(ДанныеЛиста.П000800001050, 1000000); 
	
	ДанныеЛиста.П000800001090 = ДанныеЛиста.П000800001050 + ДанныеЛиста.П000800001070; 
	ДанныеЛиста.П000800001100 = ДанныеЛиста.П000800001060 + ДанныеЛиста.П000800001080; 
	
	ДанныеЛиста.П000800001150 = ДанныеЛиста.П000800001110 + ДанныеЛиста.П000800001130; 
	ДанныеЛиста.П000800001160 = ДанныеЛиста.П000800001120 + ДанныеЛиста.П000800001140; 
	
	ДанныеЛиста.П000800001190 = ДанныеЛиста.П000800001100 + ДанныеЛиста.П000800001160 + ДанныеЛиста.П000800001180; 
	
	ПараметрыЗаполнения.ЗаполненныеРазделы.Вставить("ЛистД2", ДанныеЛиста.П000800001190 <> 0);

	ПараметрыЗаполнения.ИтогиЛистов.ЛистД2_п4 = ДанныеЛиста.П000800001190;
...
Процедура ЗаполнитьЛистЕ1(СписокСохранения, ПараметрыЗаполнения)
	...
	НужныДругиеСоцВычеты = ТаблицаСоциальныхСтраховыхВычетов.Количество() > 0
			ИЛИ ТаблицаСоциальныхВычетов.Итог("СуммаВычета") > 0
			ИЛИ ДополнительныеВзносыПФПринимаетсяКВычету <> 0;
	...
	Если НужныДругиеСоцВычеты Тогда
		...
		Если ТаблицаСоциальныхСтраховыхВычетов.Количество() > 0 Или ДополнительныеВзносыПФПринимаетсяКВычету <> 0 Тогда
...
Процедура ЗаполнитьЛистЕ2(СписокСохранения, ПараметрыЗаполнения, СуммаИтога)
	...
	Для Каждого СтрокаВзноса Из ТаблицаСоциальныхСтраховыхВычетов Цикл
		...
		ДанныеЛиста.П001000001080 = СтрокаВзноса.ВычетУНалоговогоАгента;
	...
	Если ТаблицаСоциальныхСтраховыхВычетов.Количество() = 0 Тогда
		СтраницаЛиста = ДобавитьВСписокСохраненияОтчетаСтраницуЛиста(СписокСохранения, ПараметрыЗаполнения, "ЛистЕ2");
		ДанныеЛиста   = СтраницаЛиста.Данные;
	КонецЕсли;
	...
	ДанныеЛиста.П001000001090 = ДополнительныеВзносыПФСуммаВзноса;
	ДанныеЛиста.П001000001100 = ДополнительныеВзносыПФПринимаетсяКВычету;
	ДанныеЛиста.П001000001110 = ДополнительныеВзносыПФВычетУНалоговогоАгента;
	
	ВсегоПринимается = ВсегоПринимается + ДополнительныеВзносыПФПринимаетсяКВычету;
	ВсегоУАгентов    = ВсегоУАгентов + ДополнительныеВзносыПФВычетУНалоговогоАгента;
	СуммаИтога       = ВсегоПринимается - ВсегоУАгентов;
	
	ДанныеЛиста.П001000001120 = ВсегоПринимается;
	ДанныеЛиста.П001000001130 = ВсегоУАгентов;
	ДанныеЛиста.П001000001140 = СуммаИтога;

	ПараметрыЗаполнения.ИтогиЛистов.ЛистЕ2_п3 = ДанныеЛиста.П001000001130;
...
Процедура УчестьДоходНаЛистеЖ(СписокСохранения, ПараметрыЗаполнения, КодДохода, СуммаДохода, КодВычета, СуммаВычета, КодИсточника = 1)
	...
	ИначеЕсли КодДохода = "1540" Тогда // Продажа доли в уставном капитале организации
	...
	ИначеЕсли КодДохода = "1550" Тогда // Устапка прав требования в договое долевого строительства
		...
		ДанныеЛиста.П001100001090 = ДанныеЛиста.П001100001090 + СуммаДохода;
		ДанныеЛиста.П001100001100 = ДанныеЛиста.П001100001100 + ВычетДляЗаписи;
		
	КонецЕсли; 
	
	// Подсчет итога
	ДанныеЛиста.П001100001110 = ДанныеЛиста.П001100001020 + ДанныеЛиста.П001100001040
				+ ДанныеЛиста.П001100001060 + ДанныеЛиста.П001100001080 + ДанныеЛиста.П001100001100;
	
	ПараметрыЗаполнения.ИтогиЛистов.ЛистЖ_п6_1 = ПараметрыЗаполнения.ИтогиЛистов.ЛистЖ_п6_1 + ДанныеЛиста.П001100001110;
...
Процедура ЗаполнитьИтогиЛистаЗ(СписокСохранения, ПараметрыЗаполнения)
	...
	ЛистЗПродолжение.П001300001203 = Макс(0, ЛистЗПродолжение.П001300001201 - ЛистЗПродолжение.П001300001202 - ЛистЗПродолжение.П001300001203);
...
Процедура ЗаполнитьЛистД1(СписокСохранения, ПараметрыЗаполнения)
	...
	ВычетДоИмущества = ПараметрыЗаполнения.ИтогиЛистов.ЛистГ_п10 // п. 10 Листа Г
				...
				+ ПараметрыЗаполнения.ИтогиЛистов.ЛистЕ2_п3
				...
				+ ПараметрыЗаполнения.ИтогиЛистов.ЛистЖ_п6_1;
...
Процедура ЗаполнитьРаздел2(СписокСохранения, ПараметрыЗаполнения)
	...
	// Сворачиваем таблицу доходов:
	СводДоходов = ПараметрыЗаполнения.СводДоходов;
	СводДоходов.Свернуть("СтавкаНалога,ОКТМО,КБК", "СуммаДохода,СуммаВычета,НалогУдержан,НалогУплачен,НалогПринимаемыйКУчету");
	...
	Для Каждого СтавкаНалога Из СписокСтавок Цикл
		...
		Если СтавкаНалога = 13 И НалогоплательщикСтатус = 1 Тогда
			...
			// Сумма налоговых вычетов (пп. 3.2 Листа В 
			//	+ пп. 2.5 Листа Д1 + пп. 2.6 Листа Д1 + пп. 2.8 Листа Д1 + пп. 2.9 Листа Д1 
			//	+ п. 4 Листа Д2 + п. 4 Листа Е1 + п. 3 Листа Е2 + пп. 6.1 Листа Ж) 
			ДанныеЛиста.П000200001040 = ИтогиЛистов.ЛистВ_п3_2 + ИтогиЛистов.ЛистД1_п2_5 + ИтогиЛистов.ЛистД1_п2_6
										 ...
										 + ИтогиЛистов.ЛистЕ1_п4 + ИтогиЛистов.ЛистЕ2_п3 + ИтогиЛистов.ЛистЖ_п6_1;
		...
		ДанныеЛиста.П000200001030 = ДанныеЛиста.П000200001010 - ДанныеЛиста.П000200001020; // 3. Общая сумма дохода, подлежащая налогообложению 
		
		ДанныеЛиста.П000200001060 = Макс(0, ДанныеЛиста.П000200001030 - ДанныеЛиста.П000200001040 - ДанныеЛиста.П000200001050); // 6. Налоговая база для исчисления налога
		ДанныеЛиста.П000200001070 = Окр(ДанныеЛиста.П000200001060 * СтавкаНалога / 100, 0); // 7. Общая сумма налога, исчисленная к уплате
		
		ДанныеЛиста.П000200001100 = ИтогиЛистов.ЛистВ_п3_4; // 10. Сумма налога, уплаченная налогоплательщиком, подлежащая зачёту (авансовые платежи предпринимателя)
		...
		ДанныеЛиста.П000200001130 = Макс(0, ДанныеЛиста.П000200001070 - ДанныеЛиста.П000200001080 - ДанныеЛиста.П000200001090
...
Процедура ЗаполнитьРаздел1(СписокСохранения, ПараметрыЗаполнения)
	...
	Если СписокСохранения.ДанныеМногостраничныхРазделов.Раздел1.Количество() = 0 Тогда
		ДобавитьВСписокСохраненияОтчетаСтраницуЛиста(СписокСохранения, ПараметрыЗаполнения, "Раздел1");
	КонецЕсли; 
...
Процедура ВывестиБланкДекларации(ТипБланка, БезПоказа = Ложь)
	...
	ФормаОтчета = ПолучитьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "ПомощникЗаполнения2015кв1", ПараметрыФормы.мВыбраннаяФорма), ПараметрыФормы);
...
Процедура ПродолжитьБезПомощникаЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Если НЕ Модифицированность Тогда
			...
			ПараметрыФормы = ПараметрыФормыРеглОтчета();
			ПараметрыФормы.Вставить("БезОткрытияФормы", Ложь);
			ОткрытьФорму("Отчет.РегламентированныйОтчет3НДФЛ.Форма.ФормаОтчета2015кв1", ПараметрыФормы);
...
Функция ПараметрыФормыРеглОтчета()
	...
	ПараметрыДляФормыОтчета.Вставить("мВыбраннаяФорма",          "ФормаОтчета2015кв1");
...
Функция ВыполнитьЗагрузкуДанныхИзДругогоОтчета(СсылкаНаОтчет, ТекстСообщений)
	...
	Попытка
		...
		Если ВерсияОтчета = "01/01/2014" Тогда
			...
			КонецЕсли; 
		ИначеЕсли ВерсияОтчета = "01/01/2015" Тогда
			Если ЕстьПомощник Тогда
				ЗагрузитьДанныеПомощника01012015(ДанныеОтчета.ДанныеПомощника);
			Иначе
				// Помощник не использовался. Загружаем данные непосредственно из отчета:
...
&НаСервере
Процедура ЗагрузитьДанныеПомощника01012015(ДанныеПомощника)

	ЗаполитьФормуИзДанныхПомощника(ДанныеПомощника);
...
Функция ПолучитьЗначениеИзМакетаПоКоду(Код, ИмяОбласти)
	...
	Если Год(ДатаПодписи) = 2015 Тогда
		ИмяФормыСписка = "Списки2015кв1";
	ИначеЕсли Год(ДатаПодписи) = 2014 Тогда
		ИмяФормыСписка = "Списки2014кв1";
	Иначе
		ИмяФормыСписка = "Списки2011кв1";
	КонецЕсли; 
