...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	ИначеЕсли ИмяПоказателя = "ИНН" Тогда
		...
		СтруктураДанныхТитульный.Вставить("ИНН", ЗначПоказателя);
	// Конец ДЕНЬГИ
...
&НаКлиенте
Процедура ПроверитьВыгрузку(Команда) Экспорт
...
&НаКлиенте
Процедура ПоказатьСДвухмернымШтрихкодомPDF417(Команда) Экспорт
...
&НаКлиенте
Процедура ПоказатьБланк(Команда) Экспорт
...
&НаКлиенте
Процедура ПечататьСразуСДвухмернымШтрихкодомPDF417(Команда) Экспорт
...
&НаКлиенте
Процедура ПечататьСразу(Команда) Экспорт
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	// ДЕНЬГИ РегламентированнаяОтчетность.ДобавитьКнопкуПрисоединенныеФайлы(ЭтаФорма);
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("мПечатьБезШтрихкодаРазрешена", Истина); // ДЕНЬГИ
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС, КПП, Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт	
	...
	// ДЕНЬГИ
	ОбновитьСсылкуОрганизации(СтруктураРеквизитовФормы, СтруктураДанныхТитульный, КодИФНС);
	ОргСтр = " (" + СтруктураРеквизитовФормы.НаимОрганизации + ")";
	Заголовок = "Декларация 3-НДФЛ " + СтруктураРеквизитовФормы.СтрПериодОтчета + ОргСтр;
	// Конец ДЕНЬГИ
...
Процедура СохранитьНаКлиентеСВопросом(Автосохранение, Вариант, ВыполняемоеОповещение, ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки, ПодобныйОтчетСуществует, КодИФНС, КПП)
	...
	Если ПодобныйОтчетСуществует Тогда
		...
		// ДЕНЬГИ
		ТекстВопроса = НСтр("ru = 'Для налогоплательщика %1 за %2 год уже существует декларация с номером корректировки %3.
				|Сохранить еще один экземпляр декларации?'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, СтруктураРеквизитовФормы.Организация,
					Формат(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ЧДЦ=; ЧГ="), Вариант);
...
Функция СформироватьИдФайл(ПараметрыВыгрузки)
		...
		Возврат ПараметрыВыгрузки.ПрефиксИмениФайлаВыгрузки
				...
				+ "_" + ПараметрыВыгрузки.КодИФНС
				// ДЕНЬГИ
				// Если ИНН не указан, нужно заполнять 12-ю нулями
				//+ "_" + СокрЛП(ПараметрыВыгрузки.ИННФЛ)
				+ "_" + ?(ПустаяСтрока(ПараметрыВыгрузки.ИННФЛ), "000000000000", СокрЛП(ПараметрыВыгрузки.ИННФЛ))
				// Конец ДЕНЬГИ
...
Процедура ОбновитьСсылкуОрганизации(СтруктураРеквизитовФормы, Знач СтруктураДанныхТитульный, Знач КодИФНС)

	СтруктураРеквизитовФормы.Организация =  Обработки.ПомощникЗаполнения3НДФЛ.ПолучитьСсылкуНалогоплательщика(
					СтруктураДанныхТитульный.Фамилия,
					СтруктураДанныхТитульный.Имя,
					СтруктураДанныхТитульный.Отчество,
					СтруктураДанныхТитульный.ИНН,
					КодИФНС, "", Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено);
	СтруктураРеквизитовФормы.НаимОрганизации = Строка(СтруктураРеквизитовФормы.Организация);
