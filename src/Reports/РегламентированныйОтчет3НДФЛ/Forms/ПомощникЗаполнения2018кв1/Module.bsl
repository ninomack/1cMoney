////////////////////////////////////////////////////////////////////////////////
//	Помощник заполнения декларации за 2017 год
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьФормуИзОтчета(ДопРеквизитыФормы.мСохраненныйДок);
	...
	Для каждого СтрокаДокумента Из Форма.СписокДокументов Цикл
		...
		Если СтрокаДокумента.ВидДокумента = "ДоходПродажаНедвижимости" И СтрокаДокумента.СтруктураДокумента.ВидВычета = "Норматив" Тогда
			...
			Иначе
				РассчетныйВычет = Форма.ДопРеквизитыФормы.ПределыВычетов.ПродажаЖилойНедвижимости;
			КонецЕсли;
			РассчетныйВычет = Мин(ОстатокПеределаНедвижимости, РассчетныйВычет, СтрокаДокумента.СтруктураДокумента.СуммаДохода);
...
&НаСервере
Процедура ЗаполнитьФормуИзОтчета(ОтчетДляЗаполнения)

	Если ЗначениеЗаполнено(ОтчетДляЗаполнения) Тогда
		// Получение/проверка года отчета из источника заполнения
		ГодОтчета = Год(ОтчетДляЗаполнения.ДатаОкончания);
		Если Не ЗначениеЗаполнено(ГодОтчета) Или ГодОтчета < 2017 Тогда
			ГодОтчета = 2017;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОтчетДляЗаполнения) Тогда
		...
		ДокументОбъект = ОтчетДляЗаполнения.ПолучитьОбъект();
		ДанныеОтчета   = ДокументОбъект.ДанныеОтчета.Получить();
		Если ДокументОбъект.ИспользоватьПомощникЗаполнения Тогда
...
Процедура ПродолжитьЗаполнениеВручную()
	...
	ПараметрыФормы = ПараметрыФормыРеглОтчета();
	ПараметрыФормы.Вставить("БезОткрытияФормы", Ложь);
	ОткрытьФорму("Отчет.РегламентированныйОтчет3НДФЛ.Форма.ФормаОтчета2018кв1", ПараметрыФормы);
