...
Процедура ПриОбработкеПараметраФормыОтбор(Отбор, ЭтаФорма) Экспорт
	...
	Если Отбор.Свойство("НазначениеРасхода") Тогда
		ЭлементыОтбора = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(КомпоновщикНастроек.ФиксированныеНастройки.Отбор, "НазначениеРасхода");
		Для каждого ЭлементОтбора Из ЭлементыОтбора Цикл
			ТипЗначенияОтбора = ТипЗнч(ЭлементОтбора.ПравоеЗначение);
			Если ТипЗначенияОтбора = Тип("СправочникСсылка.СтатьиРасходов")
