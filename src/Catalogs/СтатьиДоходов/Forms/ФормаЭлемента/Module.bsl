...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Записан элемент справочника", Новый Структура("ИмяСправочника,Ссылка,ВладелецФормы", "СтатьиДоходов", Объект.Ссылка, ?(ТипЗнч(ВладелецФормы) = Тип("ФормаКлиентскогоПриложения"), ВладелецФормы.УникальныйИдентификатор, Неопределено)));
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если Объект.Предопределенный Тогда
		...
		Если Объект.Ссылка = ПредопределенноеЗначение("Справочник.СтатьиДоходов.ИзменениеОстатка") Тогда
			ТекстПодсказки = НСтр("ru = 'Статья используется программой для операций ввода/изменения остатков. 
										|Наименование изменять нельзя.'");
			
		ИначеЕсли Объект.Ссылка = ПредопределенноеЗначение("Справочник.СтатьиДоходов.ОбменВалюты") Тогда
...
Функция КоличествоПодчиненныхЭлементовСервер(Родитель, ТолькоАктивные = Истина)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(СтатьиДоходов.Ссылка) КАК КоличествоПодчиненных
	|ИЗ
	|	Справочник.СтатьиДоходов КАК СтатьиДоходов
	|ГДЕ
	|	СтатьиДоходов.Ссылка В ИЕРАРХИИ(&Родитель)
	|	И СтатьиДоходов.Ссылка <> &Родитель
	|	И (НЕ &ТолькоАктивные
	|			ИЛИ &ТолькоАктивные
...
Процедура ЗапроситьПодтверждениеКомандыЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Если ДополнительныеПараметры.ИмяКоманды = "СкопироватьНастройкиАналитики" Тогда
			...
			Если НЕ ДополнительныеПараметры.Свойство("ЗакрыватьФорму") Тогда
				ПоказатьПредупреждение(,НСтр("ru = 'Настройки аналитики скопированы'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) );
			КонецЕсли; 
		КонецЕсли; 
	...
	Попытка
		...
		Иначе
			...
			Для Каждого ВидАналитики Из ВидыАналитикиДляИзменения Цикл
				...
				ОбъектВидаАналитики.РасширенныеНастройкиДляДохода = Истина;
...
Функция ВидыАналитикБезРасширенныхНастроек(СписокВидовАнилитик)
	...
	|	И НЕ ВидыАналитик.РасширенныеНастройкиДляДохода";
...
Процедура ПередЗакрытиемВопросЗавершение(Ответ = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ПроверятьПодчиненные И АналитикаИзменена Тогда
