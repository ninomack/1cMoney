...
Процедура КомандаПроверитьПодключение(Команда)
	...
	ТекстРезультата = "";
	Если ПроверитьДоступностьDropBox(ТекстРезультата) Тогда
...
&НаСервере
Функция ПроверитьДоступностьDropBox(ОписаниеПроблемы)

	ОписаниеПроблемы = "";
	Если Объект.ЗапретитьИспользование Тогда
		ОписаниеПроблемы = НСтр("ru = 'Выключено использование Dropbox'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Возврат Ложь;
	КонецЕсли;
