...
Процедура FILEКаталогОбменаИнформациейНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбменДаннымиКлиент.ОбработчикВыбораФайловогоКаталога(ЭтотОбъект, "ЯндексДискКаталогОбменаИнформацией", СтандартнаяОбработка);
...
Процедура КомандаПроверитьПодключение(Команда)
	...
	ТекстРезультата = "";
	Если ПроверитьКаталогЯндексДискаСервер(ТекстРезультата) Тогда
		ТекстРезультата = НСтр("ru = 'Каталог на Яндекс.Диске готов к обмену данными'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли; 
...
Процедура ЗаписатьПараметрыТранспорта(ТекущийОбъект)
	...
	НоваяСтрокаПараметровТранспорта(ТекущийОбъект, "ЯндексДискКаталогОбменаИнформацией", ЯндексДискКаталогОбменаИнформацией);
...
&НаСервере
Функция ПроверитьКаталогЯндексДискаСервер(ОписаниеПроблемы)

	ОписаниеПроблемы = "";
	Если Объект.ЗапретитьИспользование Тогда
		ОписаниеПроблемы = НСтр("ru = 'Выключено использование Яндекс.Диск'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Возврат Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ЯндексДискКаталогОбменаИнформацией) Тогда
		ОписаниеПроблемы = НСтр("ru = 'Не указан каталог на Яндекс.Диске'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
