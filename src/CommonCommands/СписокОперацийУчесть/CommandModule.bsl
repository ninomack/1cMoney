...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Счетчик           = 0;
	ОписаниеПроблемы  = "";
	ТекстСостояния    = НСтр("ru='Учет операций %1 из %2'"); 
	...
	Для каждого Ссылка Из ВыделенныеСтроки Цикл
		...
		Счетчик = Счетчик + 1;
		Если Не ОбслуживаниеДокументовВызовСервера.ПровестиДокументПоСсылке(Ссылка, ОписаниеПроблемы, ВсегоСтрок = 1) Тогда
